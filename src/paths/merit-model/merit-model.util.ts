...
function deriveEvidence(entry: CourseProjectEntry): {
  evs: string;
  num: string;
  numOk: 0 | 1;
} {
  ...
  const numOk: 0 | 1 = metrics.some(
    (m) => nonEmpty(m.value) && !nonEmpty(m.status),
  )
    ? 0
    : 1;
  ...
  const measured = resultsInfo?.measured
    ? stripEmojiPrefix(resultsInfo.measured)
    : '';
  const findingsCount = resultsInfo?.findings?.length ?? 0;
  if (
    (measured.startsWith('Yes') || measured.startsWith('Partly')) &&
    findingsCount > 0
  ) {
    return { evs: 'Qualitative evidence', num: '', numOk };
  }
  if (measured.startsWith('Not yet'))
    return { evs: 'Not measured yet', num: '', numOk };
  if (entry.moduleInclusion?.find === false && findingsCount === 0)
    return { evs: 'Not applicable', num: '', numOk };
  return { evs: 'Not measured yet', num: '', numOk };
}
...
export function extractMeritInputs(entry: CourseProjectEntry): MeritInputs {
  ...
  const format =
    assignmentInfo?.formats?.[0] ||
    assignmentInfo?.format ||
    'Unspecified format';

  const aimWords = wordCount(aimsInfo?.aimStatement);
  const aim: 0 | 1 | 2 =
    aimWords === 0 ? 0 : aimWords < AIM_CLEAR_MIN_WORDS ? 1 : 2;

  const learnWords = wordCount(reflectionInfo?.lessonLearned);
  const learn: 0 | 1 | 2 =
    learnWords === 0
      ? 0
      : learnWords < REFLECTION_SUBSTANTIVE_MIN_WORDS
        ? 1
        : 2;

  const hasMethods =
    (processInfo?.methods?.length ?? 0) > 0 ||
    nonEmpty(processInfo?.methodsOther);
  const hasActivities =
    (processInfo?.activities?.length ?? 0) > 0 ||
    nonEmpty(processInfo?.activitiesOther);
  const fit: 0 | 1 | 2 =
    hasMethods && hasActivities ? 2 : hasMethods || hasActivities ? 1 : 0;

  const findingsCount = resultsInfo?.findings?.length ?? 0;
  const hasSummary = nonEmpty(resultsInfo?.resultsSummary);
  const insight: 0 | 1 | 2 =
    findingsCount >= 2 ? 2 : findingsCount === 1 || hasSummary ? 1 : 0;

  const hasLimitation =
    nonEmpty(resultsInfo?.limitationType) ||
    nonEmpty(resultsInfo?.limitationDetail);
  const hasLimitationEffect = nonEmpty(resultsInfo?.limitationInterpretation);
  const lim: 0 | 1 | 2 =
    hasLimitation && hasLimitationEffect ? 2 : hasLimitation ? 1 : 0;

  const { evs, num, numOk } = deriveEvidence(entry);

  const primary = sdgMapping?.notApplicable
    ? undefined
    : sdgMapping?.entries?.[0];
  const sdg: { p: number | undefined; target: 0 | 1; how: 0 | 1 } = {
    p: primary?.goalNumber,
    target: (primary?.targets?.length ?? 0) > 0 ? 1 : 0,
    how: nonEmpty(primary?.how) ? 1 : 0,
  };
  ...
  return {
    ...
    next:
      nonEmpty(reflectionInfo?.nextSteps) || nonEmpty(reflectionInfo?.whatsNext)
        ? 1
        : 0,
    ...
    evf:
      ...
      (resultsInfo?.metrics?.some((m) => m.evidenceAttached) ?? false)
        ? 1
        : 0,
  };
}
...
export function scorecard(c: MeritInputs): MeritScorecard {
  const task: MeritCriterionScore = {
    ...
    note:
      c.aim === 2
        ? 'Clear task, real issue named, scope defined'
        : c.aim === 1
          ? 'Task/purpose present but loosely framed'
          : 'Task or purpose unclear',
  };
  ...
  const output: MeritCriterionScore = {
    ...
    max: 15,
    note: c.output
      ? 'Output produced and described'
      : 'No output described yet',
  };
  ...
  const analysis: MeritCriterionScore = {
    pts: clamp(
      0,
      20,
      (EVPTS[c.evs] || 3) + c.insight * 5 + c.output * 3 + c.numOk * 2,
    ),
    max: 20,
  ...
  const coreAt = (min: number) =>
    ratio(method) >= min && ratio(output) >= min && ratio(analysis) >= min;
  ...
  if (
    total > 94 &&
    !(coreAt(0.9) && ratio(sustainability) >= 0.8 && ratio(reflection) >= 0.7)
  )
    total = 94;
...
export function grade(total: number): MeritGrade {
  const band =
    GRADE_BANDS.find((b) => total >= b.min) ??
  ...
  const badge =
    comp >= 0.6 && cred >= 0.6
      ? {
          ...
          color: '#16a34a',
          background: '#e8f8ee',
          description: 'tells the full story and can prove it',
        }
      : comp >= 0.6
        ? {
            icon: '🟡',
            ...
            background: '#fdf3dd',
            description:
              'many sections filled, little verifiable — volume never beats evidence',
          }
        : cred >= 0.6
          ? {
              icon: '🔵',
              ...
              background: '#e8effe',
              description:
                'fewer sections, but everything said is proven — honesty outranks volume',
            }
          : {
              icon: '⚪',
              label: 'LEAN & UNPROVEN',
              color: '#7a8095',
              background: '#f1f2f7',
              description: 'little told, little proven — scored for feedback',
            };

  return {
    completenessPercent: Math.round(comp * 100),
    credibilityPercent: Math.round(cred * 100),
    badge,
  };
}

type MeritCriterionKey = Exclude<
  keyof MeritScorecard,
  'total' | 'integrityFlag'
>;
const CRITERION_KEYS: MeritCriterionKey[] = [
  'task',
  'knowledge',
  'method',
  'output',
...
export function reason(S: MeritScorecard, c: MeritInputs): string {
  const parts = CRITERION_KEYS.map((key) => ({
    key,
    ratio: S[key].pts / S[key].max,
  }))
  ...
  const bits: Record<MeritCriterionKey, string> = {
    ...
    analysis: c.evs.includes('measured result')
      ? 'analysis backed by real measured results — fruitful, not decorative'
      : 'substantive, honestly-classified findings',
    sustainability: c.orig.toLowerCase().includes('student')
      ? 'a self-started, authentic sustainability core'
      : 'an authentic, explained sustainability core',
