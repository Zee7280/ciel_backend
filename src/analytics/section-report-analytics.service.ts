...
export class SectionReportAnalyticsService {
    ...
    async getSectionAnalytics(
        ...
        const { reports, titles } = await this.scopeService.loadScopedReports({
            ...
            facultyId:
                requester.role === 'faculty' ? requester.id : undefined,
        });

        const raw = this.computeSectionMetrics(section as ReportSectionNumber, reports, {
            stakeholder,
            scope,
            titles,
        });
    ...
    async getSummary(
        ...
        type SummarySectionRow = SummaryAnalyticsResponse['data']['sections'][number];
        ...
        const s1Pct =
            reports.length === 0
                ? 0
                : Math.round((100 * s1Ready) / reports.length);
        sections.push({
            section: 1,
            title: REPORT_SECTION_TITLES[1],
            status:
                s1Ready === 0 ? 'empty' : s1Pct >= 70 ? 'ready' : 'partial',
    ...
    private computeSectionMetrics(
        ...
        const projectTitle =
            ctx.stakeholder === 'un_government'
                ? { project_count: new Set(reports.map((r) => r.opportunityId || r.project_id)).size }
                : ctx.scope === 'project'
    ...
    private computeSection2(reports: StudentReport[], projectTitle: unknown): AnalyticsFieldValues {
        const withSec = reports.filter((r) => this.hasContent(r.section2));
        const disc = this.dist(withSec.map((r) => r.section2?.discipline || r.discipline_alignment || 'Unspecified'));
        const cats = this.dist(withSec.map((r) => r.section2?.problem_category || r.problem_category || 'Unspecified'));
        const evidence = this.dist(withSec.map((r) => r.section2?.baseline_evidence || r.baseline_evidence_source || 'Unspecified'));
        const beneficiaries = this.dist(withSec.map((r) => r.section2?.primary_beneficiary || r.primary_beneficiary || 'Unspecified'));
        const thin = withSec.filter(
            (r) =>
                !(r.section2?.problem_statement || '').trim() ||
                (r.section2?.problem_statement || '').trim().length < 40,
        ).length;
        return {
            ...
            context_quality_flags: {
                ...
                missing_baseline: withSec.filter((r) => !r.section2?.baseline_evidence).length,
            },
            red_flag_count: { count: thin, action_required: thin > 0 },
        };
    }

    private computeSection3(reports: StudentReport[], projectTitle: unknown): AnalyticsFieldValues {
        const withSec = reports.filter((r) => this.hasContent(r.section3) || r.primary_sdg_goal != null);
        ...
        let secondary = 0;
        for (const r of withSec) secondary += r.section3?.secondary_sdgs?.length ?? 0;
        ...
        const statements = withSec.filter(
            (r) =>
                (r.section3?.contribution_intent_statement || r.contribution_intent_statement || '')
                    .trim().length >= 40,
        ).length;
        const weak = withSec.filter((r) => r.sdg_validation_status === 'weak').length;
        return {
            project_title: projectTitle,
            section_completion_rate: this.pctObj(withSec.length, reports.length),
            reports_with_section: withSec.length,
    ...
    private computeSection4(reports: StudentReport[], projectTitle: unknown): AnalyticsFieldValues {
        const withSec = reports.filter((r) => this.hasContent(r.section4));
        const types = this.dist(withSec.map((r) => r.section4?.activity_type || 'Unspecified'));
        const modes = this.dist(withSec.map((r) => r.section4?.delivery_mode || 'Unspecified'));
        let beneficiaries = 0;
        let hours = 0;
        let sessions = 0;
        const catMap = new Map<string, number>();
        for (const r of withSec) {
            beneficiaries += this.parseNum(r.section4?.total_beneficiaries) || this.parseNum(r.section4?.my_beneficiaries) || 0;
            hours += this.parseNum(r.section4?.my_hours) || 0;
            sessions += this.parseNum(r.section4?.total_sessions) || this.parseNum(r.section4?.my_sessions) || 0;
            for (const c of r.section4?.beneficiary_categories || []) {
                catMap.set(c, (catMap.get(c) || 0) + 1);
            }
        }
        const missingBen = withSec.filter(
            (r) => !this.parseNum(r.section4?.total_beneficiaries) && !this.parseNum(r.section4?.my_beneficiaries),
        ).length;
        return {
            project_title: projectTitle,
            section_completion_rate: this.pctObj(withSec.length, reports.length),
    ...
    private computeSection5(reports: StudentReport[], projectTitle: unknown): AnalyticsFieldValues {
        const withSec = reports.filter((r) => this.hasContent(r.section5));
        const areas = this.dist(withSec.map((r) => r.section5?.outcome_area || 'Unspecified'));
        const confidence = this.dist(withSec.map((r) => r.section5?.confidence_level || 'Unspecified'));
        const both = withSec.filter(
            (r) => (r.section5?.baseline || '').trim() && (r.section5?.endline || '').trim(),
        ).length;
        const challenges = withSec.filter((r) => (r.section5?.challenges || '').trim().length >= 20).length;
        const lowConf = withSec.filter((r) =>
            /low|weak|uncertain/i.test(r.section5?.confidence_level || ''),
        ).length;
        return {
            project_title: projectTitle,
            section_completion_rate: this.pctObj(withSec.length, reports.length),
    ...
    private computeSection6(reports: StudentReport[], projectTitle: unknown): AnalyticsFieldValues {
        const withSec = reports.filter((r) => this.hasContent(r.section6));
        const timeOnly = withSec.filter(
            (r) => r.section6?.use_resources === 'no' || !(r.section6?.resources?.length),
        ).length;
        ...
        for (const r of withSec) {
            for (const res of r.section6?.resources || []) {
                entries += 1;
                const type = (res.type || 'Other').trim() || 'Other';
                typeMap.set(type, (typeMap.get(type) || 0) + 1);
                const source = (res.source || 'Unspecified').trim() || 'Unspecified';
                sourceMap.set(source, (sourceMap.get(source) || 0) + 1);
                const ver = (res.verification || 'Pending').trim() || 'Pending';
                verifMap.set(ver, (verifMap.get(ver) || 0) + 1);
                if (/self/i.test(source)) selfFunded += 1;
                const amt = this.parseNum(res.amount);
                if (amt > 0 && /pkr|rs|rupee|cash|financial/i.test(`${res.unit} ${res.type}`)) {
                    mobilized += amt;
                    if (/self|pending|report/i.test(ver) && amt >= 10000) unverifiedHigh += 1;
                } else if (amt > 0) {
                    mobilized += amt;
                }
            }
        ...
        return {
            ...
            red_flag_count: { count: unverifiedHigh, action_required: unverifiedHigh > 0 },
        };
    }

    private computeSection7(reports: StudentReport[], projectTitle: unknown): AnalyticsFieldValues {
        const withSec = reports.filter((r) => this.hasContent(r.section7));
        const withPartners = withSec.filter(
            (r) => r.section7?.has_partners === 'yes' && (r.section7?.partners?.length || 0) > 0,
        );
        const typeMap = new Map<string, number>();
        ...
        for (const r of withSec) {
            ...
            for (const p of r.section7.partners) {
                ...
                typeMap.set(p.type || 'Other', (typeMap.get(p.type || 'Other') || 0) + 1);
                const v = p.verification || 'Self-reported';
                ...
                for (const c of p.contribution || []) {
                    contribMap.set(c, (contribMap.get(c) || 0) + 1);
                }
            }
        ...
        return {
            ...
            section_completion_rate: this.pctObj(withSec.length, reports.length),
            reports_with_section: withSec.length,
            projects_with_partners_percent: this.pctObj(withPartners.length, withSec.length || 1),
    ...
    private computeSection8(reports: StudentReport[], projectTitle: unknown): AnalyticsFieldValues {
        const withSec = reports.filter((r) => this.hasContent(r.section8));
        const backed = withSec.filter((r) => (r.section8?.evidence_types?.length || 0) > 0);
        const typeMap = new Map<string, number>();
        ...
        for (const r of withSec) {
            ...
            const ethCount = eth
                ? [eth.authentic, eth.informed_consent, eth.no_harm, eth.privacy_respected].filter(Boolean)
                      .length
                : 0;
        ...
        return {
            ...
            partner_verified_percent: this.pctObj(partnerVerified, withSec.length || 1),
            ...
            red_flag_count: { count: bypass + consentRisk, action_required: bypass + consentRisk > 0 },
        };
    }

    private computeSection9(reports: StudentReport[], projectTitle: unknown): AnalyticsFieldValues {
        ...
        let cog = 0, prac = 0, soc = 0, trans = 0, n = 0, ratingSum = 0, ratingCount = 0;
        let specific = 0;
        let curricular = 0;
        let flags = 0;
        ...
        for (const r of withSec) {
            ...
            if (scores) {
                ...
                n += 1;
                for (const v of [scores.cognitive, scores.practical, scores.social, scores.transformative]) {
                    if (typeof v === 'number') {
                        ratingSum += v;
                        ratingCount += 1;
                    }
                }
                const vals = [scores.cognitive, scores.practical, scores.social, scores.transformative];
                if (vals.every((v) => v === vals[0]) && vals[0] != null) flags += 1;
            }
            const text = `${r.section9?.personal_learning || ''} ${r.section9?.academic_application || ''}`;
            if (text.trim().length >= 80 && !/improved my communication and teamwork/i.test(text)) {
                specific += 1;
            }
            if (/course|credit|capstone|research/i.test(r.section9?.academic_integration || '')) {
                curricular += 1;
            }
            if (!(r.section9?.academic_application || '').trim()) flags += 1;
        }
        ...
        return {
            ...
            curricular_integration_percent: this.pctObj(curricular, withSec.length || 1),
    ...
    private computeSection10(reports: StudentReport[], projectTitle: unknown): AnalyticsFieldValues {
        ...
        const yes = withSec.filter((r) => r.section10?.continuation_status === 'yes').length;
        ...
        for (const r of withSec) {
            ...
            if ((r.section10?.policy_influence || '').trim().length >= 20) policy += 1;
            if (
                r.section10?.continuation_status === 'no' ||
                !(r.section10?.continuation_details || '').trim()
            ) {
                noPlan += 1;
            }
        }
    ...
    private headlineForSection(section: number, metrics: AnalyticsFieldValues): string {
        const withSec =
            typeof metrics.reports_with_section === 'number'
                ? metrics.reports_with_section
                : 0;
        const completion =
            typeof (metrics.section_completion_rate as { percent?: number })?.percent ===
            'number'
                ? (metrics.section_completion_rate as { percent: number }).percent
                : 0;
        switch (section) {
            ...
            case 7: {
                const p = (metrics.projects_with_partners_percent as { percent?: number })
                    ?.percent;
                return `${p ?? 0}% projects with partners`;
            }
            case 8: {
                const p = (metrics.evidence_backed_percent as { percent?: number })?.percent;
                return `${p ?? 0}% evidence-backed`;
            }
            case 9:
                return `Avg competency ${metrics.avg_self_rating ?? '—'} · ${completion}% complete`;
            case 10: {
                const p = (metrics.continuation_yes_rate as { percent?: number })?.percent;
    ...
    private pickSummaryKeys(section: number, metrics: AnalyticsFieldValues): Record<string, unknown> {
        const keysBySection: Record<number, string[]> = {
            ...
            8: ['evidence_backed_percent', 'avg_credibility_score', 'partner_verified_percent'],
    ...
    private credibilityScore(r: StudentReport): number {
        ...
        if (eth) {
            score +=
                [eth.authentic, eth.informed_consent, eth.no_harm, eth.privacy_respected].filter(
                    Boolean,
                ).length * 4;
        }
        if (types >= 2) score += 9;
    ...
    private pctObj(n: number, d: number): { percent: number; numerator: number; denominator: number } {
    ...
    private dist(labels: string[]): DistRow[] {
        const map = new Map<string, number>();
        for (const l of labels) {
            const key = (l || 'Unspecified').trim() || 'Unspecified';
