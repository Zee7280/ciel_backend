...
export function buildOpportunityDetailView(
  ...
  const act =
    opp.activity_details && typeof opp.activity_details === 'object'
      ...
      : {};
  const obj =
    opp.objectives && typeof opp.objectives === 'object'
      ? (opp.objectives as Record<string, unknown>)
      : {};
  const loc =
    opp.location && typeof opp.location === 'object'
      ? (opp.location as Record<string, unknown>)
      : {};
  const timeline =
  ...
  const partnerOrg =
    ...
    pickStr(
      (opp.partner_organization as Record<string, unknown> | undefined)
        ?.organization_name,
    ) ||
    pickStr(
      (
        opp.external_partner_collaboration as
    ...
    pickStr(
      (
        (opp.executing_context as Record<string, unknown> | undefined)
  ...
  const beneficiariesType = Array.isArray(obj.beneficiaries_type)
    ? (obj.beneficiaries_type as unknown[])
        .map((x) => String(x))
        .filter(Boolean)
    : [];
  ...
  return {
    overview: {
      ...
      types: Array.isArray(opp.types)
        ? opp.types.filter((x): x is string => typeof x === 'string')
        : [],
    ...
    verification_method: Array.isArray(opp.verification_method)
      ? opp.verification_method.filter(
