...
export function computeVentureGates(
  v: Partial<VentureEntry>,
): VentureGateStatus {
  const academicOk = !!(
  ...
  const teamConsentOk =
    !v.teamConsent?.length || v.teamConsent.every((t) => t.consented);
  const sdgOk =
  ...
  const raising = !!(
    v.evidenceInfo?.fundingSought && v.evidenceInfo?.useOfFunds?.trim()
  );
  const stage = v.stage || '';
...
export function deriveVentureIsVisible(v: Partial<VentureEntry>): boolean {
  ...
  return (
    showcaseOk &&
    !!v.publishSettings?.audience &&
    v.publishSettings.audience !== 'private'
  );
}
