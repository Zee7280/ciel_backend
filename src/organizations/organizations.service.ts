...
export class OrganizationsService {
   ...
   */
  private async resolveUniversityAffiliatedStudentIds(
    orgId: string,
    orgName: string,
  ): Promise<Set<string>> {
    ...
    const rows = await this.participationRepository
      ...
      .andWhere('p.status IN (:...st)', {
        st: UNIVERSITY_SCOPED_PARTICIPATION_STATUSES,
      })
      .andWhere(
        new Brackets((b) => {
          b.where(`TRIM(COALESCE(p.universityId, '')) = :orgId`, { orgId })
            ...
            .orWhere(
              `LOWER(TRIM(COALESCE(p.universityId, ''))) = :orgNameNorm`,
              { orgNameNorm },
            )
            .orWhere(
              `LOWER(TRIM(COALESCE(student.institution, ''))) = :orgNameNorm`,
              { orgNameNorm },
            )
            .orWhere(
  ...
  private async countApplicantsByOpportunity(
    oppIds: string[],
   ...
   */
  private async getPartnerDashboardStatsForUniversityOrg(
    orgId: string,
    orgName: string,
  ) {
    const oppIds =
            ...
            items: [
              {
                ...
                tone: 'warning',
                description:
                  'Student hours or reports waiting for partner review.',
              },
              {
                ...
                tone: 'neutral',
                description:
                  'Your opportunities still moving through approval.',
              },
            ],
          },
          recentProjects: [],
    ...
    const affiliatedStudentIds =
      await this.resolveUniversityAffiliatedStudentIds(orgId, orgName);
    ...
    const timesheets = timesheetsAll.filter(
      (t) => t.studentId && affiliatedStudentIds.has(t.studentId),
    );
    ...
    const reportsSubmitted = affiliatedStudentIds.size
      ? await this.reportsRepository
          ...
          .andWhere('r."studentId" IN (:...studentIds)', {
            studentIds: [...affiliatedStudentIds],
          })
          .getCount()
      : 0;
    ...
    const applicantCounts = await this.countApplicantsByOpportunity(
    ...
    const verificationPercentage =
      decidedCount === 0 ? 0 : Math.round((100 * verifiedCount) / decidedCount);
      ...
      data: {
        ...
        verificationProgress: {
          ...
          label:
            decidedCount === 0
              ? 'No submissions yet'
              : `${verifiedCount} of ${decidedCount} verified`,
        },
      },
    };
  }
  ...
  async getPartnerDashboardStats(orgId: string) {
    ...
    const pendingTimesheetsForRate = timesheets.filter(
      (t) => t.status === 'pending',
    );
  ...
  async updateMyOrganization(
    ...
    if (!user.organization) {
      ...
      const orgToLink = Array.isArray(savedOrg)
        ? savedOrg[0]
        : (savedOrg as Organization);

      // Link to user explicitly if needed
      user.organization = orgToLink;
      await this.usersRepository.save(user);
      await this.maybeFillUserContactFromOrg(
        user,
      ...
      const updatedOrg = await this.organizationsRepository.findOne({
  ...
  async findAll(filters: any) {
    ...
    if (filters.status) {
      query.andWhere('org.verificationStatus = :status', {
        status: filters.status,
      });
    }
    if (filters.country) {
      query.andWhere('org.countryCode = :country', {
  ...
  async rejectOrganization(
    id: string,
    adminId: string,
    dto: AdminRejectOrganizationDto,
  ) {
    const org = await this.findOne(id);
  ...
  async addStaffMemberToUniversityOrganization(
    ...
    if (!this.isUniversityOrgType(org.orgType)) {
      throw new BadRequestException(
        'Staff members can only be added to university-type organizations',
      );
    }
  ...
  async getUniversityAnalytics(orgId: string) {
    ...
    if (!this.isUniversityOrgType(org.orgType)) {
      throw new ForbiddenException(
        'University analytics are only available for university organizations.',
      );
    }
    ...
    for (const p of participations) {
      const majorFallback = (
        (p.student as User | undefined)?.major || ''
      ).trim();
      const deg =
        (p.academicProgram || '').trim() || majorFallback || 'Unspecified';
    ...
    const team_participation_percent =
      total_participants === 0
        ? 0
        : Math.round((100 * teamEnrollmentRows) / total_participants);
  ...
  private async getPartnerStudentsAnalyticsForUniversityOrg(orgId: string) {
    ...
    const verification_rate_percent =
      total_students_assigned === 0
        ? 0
        : Math.round((100 * verified_students) / total_students_assigned);
  ...
  async getPartnerStudentsAnalytics(orgId: string) {
    ...
    for (const p of participations) {
      ...
      const majorFallbackNgo = (
        (p.student as User | undefined)?.major || ''
      ).trim();
      const deg =
        (p.academicProgram || '').trim() || majorFallbackNgo || 'Unspecified';
      degreeMap.set(deg, (degreeMap.get(deg) || 0) + 1);
  ...
  private async getPartnerImpactMetricsForUniversityOrg(
    orgId: string,
    orgName: string,
  ) {
    const oppIds =
    ...
    for (const o of opps) {
      ...
      const obj = o.objectives as {
        beneficiaries_count?: number;
        total_beneficiaries?: number;
      } | null;
      const b = Number(
        obj?.beneficiaries_count ?? obj?.total_beneficiaries ?? 0,
      );
    ...
    const verifiedTs = verifiedTsAll.filter(
      (t) => t.studentId && affiliatedStudentIds.has(t.studentId),
    );
    const totalHours = verifiedTs.reduce(
