...
export class CommunityAwardService {
    ...
    private async checkAndConsumeCommunityServiceGraderRun(
        ...
        const runCount = await this.graderRunRepo.manager.transaction(async (manager) => {
            ...
            let row = await repo.findOne({
                where: { pathKind: 'community_service', scope: 'faculty', scopeKey, academicYear },
            ...
            if (!row) {
                row = repo.create({ pathKind: 'community_service', scope: 'faculty', scopeKey, academicYear, runCount: 0 });
            }
            row.runCount = used + 1;
    ...
    async notifyFromPool(
        ...
        let graderRuns: { unlimited: boolean; used: number; limit: number } | undefined;
        if (kind === 'fac') {
            graderRuns = await this.checkAndConsumeCommunityServiceGraderRun(facultyEmail || '');
        }
        ...
        return { notified: sent, scope, kind, ...(graderRuns ? { graderRuns } : {}) };
    }
}
