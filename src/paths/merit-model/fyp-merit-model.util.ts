...
import {
  FypMeritCard,
  FypMeritCriterionScore,
  FypMeritGrade,
  FypMeritInputs,
  FypMeritQualityProfile,
  FypMeritScorecard,
} from './fyp-merit-model.types';
...
export function deriveFypRoute(entry: FypEntry): string {
  ...
  if (
    ...
    return 'scholar';
  if (v9 === 'prototype' || v9 === 'design' || v9 === 'creative')
    return 'maker';
  ...
  if (projectInfo?.leadRoute) {
    ...
    if (
      ['scholar', 'maker', 'builder', 'storyteller', 'consultant'].includes(
        lead,
      )
    )
      return lead;
...
function deriveRouteBlockCompleteness(
  route: string,
...
function deriveExternalEngagement(
  route: string,
  routeDetails?: FypRouteDetails | null,
): 0 | 1 {
  if (!routeDetails) return 0;
...
export function extractFypMeritInputs(entry: FypEntry): FypMeritInputs {
  ...
  const hasMethods =
    ...
    nonEmpty(methodology?.methodsOther);
  const fit: 0 | 1 | 2 =
    hasApproaches && hasMethods ? 2 : hasApproaches || hasMethods ? 1 : 0;

  const scaleEntriesCount = methodology?.scaleEntries?.length ?? 0;
  const scn =
    scaleEntriesCount > 0
      ? scaleEntriesCount
      : nonEmpty(methodology?.sampleScale)
        ? 1
        : 0;
  ...
  const evs = evsRaw
    ? (FYP_EVIDENCE_STATUS_TO_EVS[evsRaw] ?? evsRaw)
    : 'Not yet';
  ...
  const limitationText =
    findings?.limitation ||
    findings?.limitationDetail ||
    findings?.limitationType ||
    '';
  const limWords = wordCount(limitationText);
  const lim: 0 | 1 | 2 =
    limWords === 0 ? 0 : limWords < LIMITATION_SUBSTANTIVE_MIN_WORDS ? 1 : 2;
  ...
  const fwd: 0 | 1 =
    nonEmpty(whatsNext) &&
    !whatsNext!.trim().toLowerCase().startsWith('completed')
      ? 1
      : 0;

  const repo: 0 | 1 =
    (entry.deliverables?.length ?? 0) > 0 || nonEmpty(repository?.externalLinks)
      ? 1
      : 0;
  const teamMembers = projectInfo?.teamMembers ?? [];
  const linked: 0 | 1 = teamMembers.some(
    (tm): tm is FypTeamMember =>
      typeof tm === 'object' && tm !== null && nonEmpty(tm.email),
  )
    ? 1
    : 0;
...
export function fypScorecard(c: FypMeritInputs): FypMeritScorecard {
    ...
    note:
      (c.nov === 2
        ...
        : c.nov === 1
          ...
          : 'An incremental take — ') +
      (c.aim === 2
        ? 'purpose anyone can understand, concrete objectives'
        : 'purpose loosely framed'),
  };
  ...
  const rigor: FypMeritCriterionScore = {
    ...
    note:
      (c.fit === 2
        ? `Method fits the ${c.route} route`
        : c.fit === 1
          ? 'Method partly matches the route'
          : 'Process thinly evidenced') +
      (c.scn
        ? ` · ${c.scn} scale entr${c.scn === 1 ? 'y' : 'ies'} via the standard builder`
        : ' · no scale declared (may be honest N/A)') +
  ...
  const outcome: FypMeritCriterionScore = {
    pts: Math.min(
      20,
      (EVPTS[c.evs] || 3) + Math.min(c.fnd, 3) * 2.2 + Math.min(c.mm, 2) * 1.5,
    ),
    max: 20,
  ...
  let flag =
    '✅ Consistency: claims match declared evidence — every number carries its status tag.';
  if (c.evs === 'Measured / tested' && c.mm === 0) {
    hon -= 3;
    flag =
      '⚠️ Claims "measured / tested" but no MEASURED-tagged metric on the card — 3 points deducted.';
  }
  if (
    c.route === 'maker' &&
    ...
    hon -= 3;
    flag =
      '⚠️ "Work itself is evidence" claimed but no documentation/degree-show details — 3 points deducted.';
  }
  const honesty: FypMeritCriterionScore = {
    ...
    note:
      c.lim === 2
        ? 'Limitation named with its effect'
        : c.lim === 1
          ? 'Limitation named briefly'
  ...
  const pot: FypMeritCriterionScore = {
    pts: Math.min(
      ...
      c.fwd * 3 +
        c.ext * 4 +
        (strongEvidence ? 3 : c.evs === 'Qualitative' ? 2 : 1),
    ),
    max: 10,
...
export function fypCompCred(c: FypMeritInputs): FypMeritQualityProfile {
  ...
  const credPts =
    ...
    (c.conf ? 2 : 0) +
    (strongEvidence ? 2 : 0) +
    (!(c.evs === 'Measured / tested' && c.mm === 0) ? 2 : 0);
    ...
    comp >= 0.6 && cred >= 0.6
      ...
      : comp >= 0.6
        ? {
            ...
            background: '#fdf3dd',
            description:
              'tells a full story but light on verifiable evidence — score capped by credibility, not volume',
          }
        : cred >= 0.6
          ? {
              icon: '🔵',
              ...
              background: '#e8effe',
              description:
                'says little, proves what it says — honesty outranks volume',
            }
          : {
              icon: '⚪',
              label: 'LEAN & UNPROVEN',
...
const CRITERION_KEYS: FypCriterionKey[] = [
  'purpose',
  'rigor',
  'outcome',
  'honesty',
...
export function fypReason(S: FypMeritScorecard, c: FypMeritInputs): string {
  const ranked = CRITERION_KEYS.map((key) => ({
    key,
