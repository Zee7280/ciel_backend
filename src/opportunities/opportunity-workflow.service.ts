...
export class OpportunityWorkflowService {
  ...
  initFacultyCreated(opp: Opportunity, requiresPartner: boolean): void {
    ...
    // Keep the legacy mirror fields in sync with the line above: this path self-approves at the
    // faculty line and never runs the token-link `afterFacultyVerified()` flow that would
    // otherwise be the only place these get set, so anything still reading the legacy fields
    // (rather than `facultyApprovalStatus`) would otherwise see this row as forever
    // "pending_faculty" even though it has already moved past that stage.
