...
export function membersFromReportSection1(
  section1: unknown,
  studentId?: string | null,
): number {
  const s = section1 as
...
export function pkrFromResources(
  resources:
    | { type?: unknown; unit?: unknown; amount?: unknown; source?: unknown }[]
    | undefined,
...
function isCashResource(r: { type?: unknown; unit?: unknown }): boolean {
  ...
  if (
    unit &&
    /\bhours?\b|\bhrs\b|kits?|devices?|sessions?|licenses?|\bkg\b|liters?/i.test(
      unit,
    ) &&
...
function secondaryRowsFromSection3(
 ...
 */
export function sdgsFromVerifiedReport(
  report: VerifiedReportSdgInput,
): number[] {
  const out = new Set<number>();
  const section3 =
    report.section3 &&
    typeof report.section3 === 'object' &&
    !Array.isArray(report.section3)
