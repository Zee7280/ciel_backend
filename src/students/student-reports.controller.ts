...
export class StudentReportsController {
  ...
  async saveDraft(
    ...
    return await this.studentReportsService.saveDraft(req.user.id, body, files);
  }
  ...
  async getAllReports(@Request() req, @Query() query: any) {
    ...
    if (requestedStudentId && requestedStudentId !== req.user.id) {
      throw new ForbiddenException(
        'Unauthorized to query reports for another student',
      );
    }
    return await this.studentReportsService.findAll({
      ...query,
  ...
  async getReportById(@Request() req, @Param('id') id: string) {
    ...
    return await this.studentReportsService.findOneByOpportunityOrId(
      id,
      req.user.id,
    );
  }
