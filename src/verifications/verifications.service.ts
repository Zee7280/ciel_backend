...
export class VerificationsService {
  ...
  private async assertActorOwnsVerificationItem(
    ...
    entity: {
      ...
      opportunity?: { organizationId?: string | null } | null;
    },
  ) {
    if (actor?.role === UserRole.SUPER_ADMIN) return;
    if (!actor?.id) {
      throw new ForbiddenException(
        'You are not allowed to act on this verification item',
      );
    }
    const effectiveOrgId =
      entity.organizationId || entity.opportunity?.organizationId || null;
  ...
  async approve(id: string, feedback?: string, actor?: VerificationActor) {
    let type = 'Timesheet';
    let entity: any = await this.timesheetsRepository.findOne({
      where: { id },
