...
export class StudentReportsService {
  ...
  private formatBaselineEvidenceSource(
    section2?: StudentReport['section2'],
  ): string {
  ...
  private syncSection3Metadata(report: StudentReport, section3: any): void {
    ...
    const targetCode = this.pickOptionalString(
      primarySdg?.target_code ?? primarySdg?.target_id,
    );
    const indicatorCode = this.pickOptionalString(
      primarySdg?.indicator_code ?? primarySdg?.indicator_id,
    );
    ...
    report.contribution_intent_statement =
      section3?.contribution_intent_statement;
  ...
  private getPublicReportApprovalContext(report: StudentReport) {
    const partners = Array.isArray(report.section7?.partners)
      ? report.section7.partners
      : [];
  ...
  private async isReportFeeClearedForApprovals(
  ...
  private async loadOpportunitiesForReports(
  ...
  private resolveReportProjectKeyForMerge(
    report: Pick<StudentReport, 'opportunityId' | 'project_id'>,
  ): string {
  ...
  private isReportSafeToRemoveInAdminMerge(report: StudentReport): boolean {
    const st = String(report.status || '')
      .trim()
      .toLowerCase();
    const adm = String(report.admin_status || '')
      .trim()
      .toLowerCase();
  ...
  private shallowMergeReportFields(
    keeper: StudentReport,
    donors: StudentReport[],
  ): void {
    const jsonKeys = [
      'section1',
      'section2',
    ...
    for (const donor of donors) {
      ...
      if (
        !keeper.summary_text_generated?.trim() &&
        donor.summary_text_generated?.trim()
      ) {
   ...
   */
  async adminMergeReports(dto: {
    report_ids: string[];
    keep_report_id: string;
  }) {
    const reportIds = [
      ...new Set(dto.report_ids.map((id) => id.trim()).filter(Boolean)),
    ];
    ...
    if (!reportIds.includes(keepId)) {
      throw new BadRequestException(
        'keep_report_id must be included in report_ids.',
      );
    }
    ...
    const projectKeys = new Set(
      reports
        .map((r) => this.resolveReportProjectKeyForMerge(r))
        .filter(Boolean),
    );
    ...
    const unsafe = donors.filter(
      (r) => !this.isReportSafeToRemoveInAdminMerge(r),
    );
    if (unsafe.length) {
      throw new BadRequestException(
    ...
    return {
      ...
      data: refreshed
        ? this.mapReportListing(refreshed, opportunityByProjectId)
        : null,
    };
  }
  ...
  private mapReportListing(
    ...
    const opportunity = this.resolveLinkedOpportunity(
      report,
  ...
  private buildStudentReportFeedback(report: StudentReport): string | null {
    ...
    if (meta && typeof meta === 'object' && !Array.isArray(meta)) {
      const studentFeedback = (meta as { student_feedback?: unknown })
        .student_feedback;
  ...
  private async participantMayAccessReport(
  ...
  private async resolveCanonicalTeamLeadStudentIdForScope(
    projectKey: string,
    participation: Participation,
  ): Promise<string | null> {
    const teamId = (participation.teamId || '').trim();
    ...
    if (!teamId && !applicationId) {
      if (participation.isTeamLead && participation.studentId) {
        return participation.studentId;
      }
    }
  ...
  private async resolveCanonicalLeadForScopeKey(
    scopeKey: string,
  ): Promise<string | null> {
   ...
   */
  private async filterReportsForAdminPartnerQueue(
    reports: StudentReport[],
    ...
    for (const row of participations) {
      participationByStudentProject.set(
        `${row.studentId}:${row.projectId}`,
        row,
      );
    }
    ...
    for (const report of reports) {
      ...
      const participation = participationByStudentProject.get(
        `${report.studentId}:${projectKey}`,
      );
  ...
  private mapRosterRowToSection1Participant(
    ...
    const email =
      this.pickTrimmedString(row.email) ||
      this.pickTrimmedString(student.email);
    const storedEmail = storedLead
      ? this.pickTrimmedString(storedLead.email).toLowerCase()
      : '';
    ...
    return {
      ...
      email,
      mobile:
        this.pickTrimmedString(row.mobile) ||
        this.pickTrimmedString(student.phone),
      cnic: this.engagementService.decryptCnicInternal(
        this.pickTrimmedString(row.cnic),
      ),
      university:
  ...
  private async buildLiveSection1TeamFields(report: StudentReport): Promise<{
    ...
    if (!this.looksLikeUuid(projectKey)) {
      return {
        team_members: [],
        participation_type:
          this.pickTrimmedString(storedSection1.participation_type) ||
          'individual',
      };
    }
    ...
    const isTeam =
      ...
      this.pickTrimmedString(
        storedSection1.participation_type,
      ).toLowerCase() === 'team' ||
      scoped.length > 1;
    ...
    const memberRows = scoped.filter(
      (row) => this.pickTrimmedString(row.id) !== leadId,
    );

    return {
      participation_type: 'team',
      team_lead: leadRow
        ? this.mapRosterRowToSection1Participant(leadRow, storedLead)
        : undefined,
      team_members: memberRows.map((row) =>
        this.mapRosterRowToSection1Participant(row),
      ),
    };
  }

  private pickTeamLeadReportBySection1(
    group: StudentReport[],
    ...
    for (const report of group) {
      ...
      const studentEmail = this.pickTrimmedString(
  ...
  /**
   * When multiple report rows exist for one team scope (legacy member drafts), keep one canonical row.
   * Multiple teams on the same project each keep their own lead report.
   */
  private async collapseDuplicateTeamProjectReports(
    reports: StudentReport[],
  ): Promise<StudentReport[]> {
    if (reports.length <= 1) {
      return reports;
    ...
    const byScope = new Map<string, StudentReport[]>();
    ...
    for (const report of reports) {
      ...
      const bucket = byScope.get(bucketKey) ?? [];
      bucket.push(report);
      byScope.set(bucketKey, bucket);
    }
    ...
    for (const [scopeKey, group] of byScope) {
      ...
      if (scopeKey.startsWith('individual:')) {
        collapsed.push(this.pickPreferredTeamReportRow(group));
        continue;
      }
  ...
  private summarizeTeamRosterForListing(
    ...
    const teamMembers = roster.map((row) => {
      ...
      const studentId =
        this.pickTrimmedString(student.id) ||
        this.pickTrimmedString(row.studentId) ||
        null;
    ...
    const leadRow =
      ...
      teamMembers.find(
        (m) => m.student_id && m.student_id === report.studentId,
      ) ??
      teamMembers[0];
   ...
   */
  private async loadMergedReportEntitiesForStudent(
    studentId: string,
  ): Promise<StudentReport[]> {
    ...
    for (const oid of uniqOpp) {
      const { report } = await this.resolveReportRecordForParticipantRead(
        studentId,
        oid,
      );
  ...
  async getMergedReportsForParticipant(
    studentId: string,
  ): Promise<StudentReport[]> {
  ...
  private async findLatestManualPayment(
    studentId: string,
   ...
   */
  private async assertTeamLeadMayWriteReport(
    studentId: string,
  ...
  private async assertTeamLeadMaySubmitReport(
    studentId: string,
    opportunityId: unknown,
  ): Promise<void> {
  ...
  async createReport(
    ...
    const opportunityIdFromDto =
      parsedData.opportunityId || parsedData.project_id;
    ...
    const reportOwnerId = opportunityIdFromDto
      ? await this.resolveTeamReportOwnerStudentId(
          studentId,
    ...
    report.primary_beneficiary = this.detectBeneficiary(
      report.section2?.problem_statement,
    );
    report.baseline_evidence_source = this.formatBaselineEvidenceSource(
      report.section2,
    );
    report.discipline_alignment =
      report.section2?.discipline || 'Not specified';

    report.summary_text_generated = this.generateSection2Summary(
      report,
      opportunityForSummary || undefined,
    );
    if (report.section2) {
    ...
    if (shouldSubmit) {
      ...
      if (validationIssues.length) {
        throw new BadRequestException({
          message:
            'Report cannot be submitted until required sections are complete.',
  ...
  async saveDraft(studentId: string, dto: any, files: any[]) {
    ...
    const reportOwnerId = opportunityId
  ...
  async findAll(query: any) {
    ...
    if (studentId) {
      ...
      if (organizationId) {
        rows = rows.filter(
          (r) => r.opportunity?.organizationId === organizationId,
        );
      }
      if (status) {
    ...
    mapped.sort((a, b) => {
      const aMs = new Date(
        a.report_submitted_at ?? a.submitted_at ?? a.submission_date ?? 0,
      ).getTime();
      const bMs = new Date(
        b.report_submitted_at ?? b.submitted_at ?? b.submission_date ?? 0,
      ).getTime();
      return bMs - aMs;
    });
  ...
  async findOneByOpportunityOrId(id: string, studentId: string) {
    ...
    if (!report) {
      ...
      if (
        bySlugOnly &&
        (await this.participantMayAccessReport(studentId, bySlugOnly))
      ) {
        report = bySlugOnly;
      }
    }
  ...
  private async buildReportAccessForViewer(
    viewerStudentId: string,
    report: StudentReport,
  ) {
    const projectKey = (report.opportunityId || report.project_id || '').trim();
    const roster = this.looksLikeUuid(projectKey)
      ? await this.engagementService.getProjectTeamForReportDossier(projectKey)
      : [];
  ...
  async verifyReport(
    ...
    if (action === 'unlock') {
      ...
      if (!reason?.trim()) {
        throw new BadRequestException(
          'feedback or reason is required when rejecting',
        );
      }
      report.status = 'revision';
  ...
  async checkReportStatus(studentId: string, opportunityId?: string) {
    if (!opportunityId) {
      const mergedReports =
        await this.loadMergedReportEntitiesForStudent(studentId);
  ...
  private generateSDGStage1Summary(report: StudentReport): string {
    const goal =
      report.primary_sdg_goal || report.section3?.primary_sdg?.goal_number || 4;
    const goalTitle =
      report.section3?.primary_sdg?.goal_title || 'Quality Education';
    const target =
  ...
  private classifyProblem(text: string): string {
    ...
    if (
      ...
      return 'Health Awareness Gap';
    if (t.includes('waste') || t.includes('climate') || t.includes('pollution'))
      return 'Environmental Degradation';
    if (t.includes('policy') || t.includes('governance') || t.includes('law'))
      return 'Policy / Governance Gap';
    if (
      t.includes('infrastructure') ||
      t.includes('building') ||
      t.includes('road')
    )
      return 'Infrastructure Deficiency';
    if (t.includes('gender') || t.includes('equality'))
    ...
    if (
      ...
      return 'Economic Opportunity Gap';
    if (
      t.includes('data') ||
      t.includes('system') ||
      t.includes('inefficiency')
    )
      return 'Data / System Inefficiency';
    return 'Other';
  }
  ...
  private generateSection2Summary(
    ...
    if (
      !opportunity?.location?.district &&
      opportunity?.timeline?.location_district
    ) {
  ...
  private updateReportWithFilePaths(
    report: StudentReport,
  ...
  private resolveCiiScoreFromPayload(
    ...
    const cii = (ciiIndex ?? section11?.cii_index) as
      | Record<string, unknown>
      | undefined;
    const fromCii =
    ...
    const summary =
      typeof section11?.summary_text === 'string' ? section11.summary_text : '';
    const patterns = [
  ...
  async updateReportAiScore(
    ...
    const score = this.resolveCiiScoreFromPayload(
      mergedSection11,
      body.cii_index ?? null,
    );
    if (score !== null) {
