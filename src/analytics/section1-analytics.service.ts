...
export class Section1AnalyticsService {
  ...
  async getSection1Analytics(
    ...
    const stakeholder =
      forcedStakeholder ??
      this.resolveStakeholderFromRole(requester.role, orgType);
    const scope =
    ...
    if (scope === 'project' && !query.project_id) {
      throw new BadRequestException(
        'project_id is required for project-scoped Section 1 analytics',
      );
    }
    ...
    const rawValues =
      scope === 'project'
        ...
        : await this.computeAggregateMetrics(
            stakeholder,
  ...
  private async assertAccess(
    ctx: Section1AnalyticsRequestContext,
  ): Promise<void> {
    if (ctx.stakeholder === 'ciel') {
      if (ctx.requesterRole !== UserRole.SUPER_ADMIN) {
        throw new ForbiddenException(
          'Admin access required for CIEL Section 1 analytics',
        );
      }
      return;
    }
    ...
    if (ctx.stakeholder === 'university') {
      ...
      const participations = await this.loadUniversityScopedParticipations(
        ctx.organizationId,
      );
  ...
  private async computeProjectMetrics(
    ...
    const report = primaryParticipation?.studentId
      ? await this.studentReportRepository.findOne({
          where: [
            {
              studentId: primaryParticipation.studentId,
    ...
    const teamMembers = participations.filter((p) => {
      if (primaryParticipation?.participationMode !== 'team')
        return p.id === primaryParticipation?.id;
      if (primaryParticipation.teamId)
        return p.teamId === primaryParticipation.teamId;
    ...
    const verifiedHours = await this.sumVerifiedHours(
      projectId,
      primaryParticipation?.studentId,
    );
    const requiredHours = this.resolveRequiredHours(opportunity);
    const section1 = report?.section1;
    const personalCompletion = studentUser
      ? this.computePersonalCompletion(studentUser)
      : null;
    ...
    const emailOtpSent = studentUser?.email
      ? await this.emailOtpRepository.count({
          where: { email: studentUser.email.toLowerCase() },
        })
      : 0;
    ...
    return {
      ...
      hours_status_in_header: {
        ...
        percent:
          requiredHours > 0
            ? Math.min(100, Math.round((100 * verifiedHours) / requiredHours))
            : 0,
      },
      ...
      cnic_completion: {
        completed: Boolean(
          studentUser?.cnic?.trim() || primaryParticipation?.cnicHash,
        ),
        masked_last4: primaryParticipation?.cnicLast4 ?? null,
        count:
          studentUser?.cnic?.trim() || primaryParticipation?.cnicHash ? 1 : 0,
      },
      ...
      mobile_contact_completion: {
        completed: Boolean(
          primaryParticipation?.mobile?.trim() || studentUser?.phone?.trim(),
        ),
        count:
          primaryParticipation?.mobile?.trim() || studentUser?.phone?.trim()
            ? 1
      ...
      student_id_cnic_link_completion: {
        linked: Boolean(
          studentUser?.registrationNumber?.trim() &&
          (studentUser?.cnic || primaryParticipation?.cnicHash),
        ),
        percent:
          studentUser?.registrationNumber?.trim() &&
          (studentUser?.cnic || primaryParticipation?.cnicHash)
      ...
      degree_program_distribution: this.toDistribution([
        [
          primaryParticipation?.academicProgram ||
            studentUser?.major ||
            'Unspecified',
          1,
        ],
      ...
      course_linked_participation_count:
        primaryParticipation?.academicIntegrationType === 'Course-Linked'
          ? 1
          : 0,
      ...
      team_configuration_status: teamStats.configured
        ? 'Configured'
        : 'Not Configured',
  ...
  private async computeAggregateMetrics(
    ...
    const titles =
      participations.length > 0
        ? [
            ...new Set(
              participations.map((p) => p.project?.title).filter(Boolean),
            ),
          ]
        : [];
    ...
    return {
      ...
      section_1_status: {
        ...
        in_progress_percent:
          100 - pct(acknowledged, Math.max(reports.length, 1)),
      },
      progress_mode_status: {
        progress_mode_count: reports.filter(
          (r) => (r.section1?.metrics?.hec_compliance || '') !== 'meets',
        ).length,
      },
      hours_status_in_header: {
      ...
      institutional_purpose_acknowledged: {
        ...
        percent: pct(
          acknowledged,
          Math.max(reports.length, participations.length),
        ),
      },
      verified_participation_via_audit_ready_logs: {
      ...
      academic_integration_type_distribution: this.toDistribution([
        ...integrationMap.entries(),
      ]),
      course_linked_participation_count: courseLinked,
      academic_info_completion_rate: {
        percent: pct(
          participations.filter((p) => p.academicProgram && p.yearOfStudy)
            .length,
          participations.length,
        ),
      },
      ...
      team_configuration_status: {
        configured_teams: [...teamGroups.values()].filter((g) => g.length > 1)
          .length,
        total_teams: teamGroups.size,
      },
      team_member_count: participations.length,
      team_member_university_distribution: this.toDistribution([
        ...uniMap.entries(),
      ]),
      team_member_role_distribution: this.toDistribution(
        participations.map((p) => [
          p.isTeamLead ? 'Team Lead' : 'Team Member',
          1,
        ]),
      ),
      active_contributor_count: participations.filter(
        (p) => p.participationMode === 'team',
      ).length,
      average_team_size: avgTeamSize,
      team_verification_rate: {
        fully_verified_teams: [...teamGroups.values()].filter((g) =>
          g.every(
            (m) => m.student?.identity_verified && m.student?.profile_verified,
          ),
        ).length,
        total_teams: teamGroups.size,
        percent: pct(
          [...teamGroups.values()].filter((g) =>
            g.every(
              (m) =>
                m.student?.identity_verified && m.student?.profile_verified,
            ),
          ).length,
          teamGroups.size || 1,
        ),
      ...
      team_compliance_warning: {
        warning_count: participations.filter(
          (p) => !p.emailVerified || !p.mobileVerified,
        ).length,
      },
      attendance_logging_unlock_status: {
        unlocked_count: participations.filter((p) => !p.attendanceLocked)
          .length,
      ...
      section_1_red_flag_count: {
        ...
        flagged_enrollments: participations.filter(
          (p) => !p.emailVerified || !p.mobileVerified,
        ).length,
      },
  ...
  private async loadUniversityScopedParticipations(
    orgId: string,
  ...
  private async sumVerifiedHours(
    projectId: string,
  ...
  private computePersonalCompletion(user: User | null): {
    ...
    const total =
      status.profile_missing_fields.length + (status.profile_complete ? 6 : 0);
    ...
    return {
      ...
      percent: Math.round(
        (completed_required_fields / total_required_fields) * 100,
      ),
    };
  }
  ...
  private computeTeamStats(members: Participation[]) {
    ...
    const verifiedMembers = members.filter(
      (m) =>
        m.emailVerified && m.mobileVerified && m.student?.identity_verified,
    ).length;
    ...
    const fullyVerified =
      verifiedMembers === members.length && members.length > 0;
    const complianceWarning =
      members.some(
        (m) => !m.emailVerified || !m.mobileVerified || !m.studentId,
      ) || false;
    return {
      memberCount,
      verifiedMembers,
      ...
      complianceWarning: complianceWarning
        ? {
            active: true,
            message: 'One or more team members are unregistered or unverified.',
          }
        : { active: false },
  ...
  private resolveAttendanceUnlock(
    ...
    const effective = resolveParticipationForAttendanceUnlock(
      participation,
      teamMembers,
    );
  ...
  private async countDuplicateCnicHashes(
  ...
  private countRedFlags(
    ...
    if (this.resolveIdentityStatus(user, participation) === 'pending')
      count += 1;
