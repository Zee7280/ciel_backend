...
export class StudentsService {
  ...
  private async getOccupiedSeats(opportunityId: string): Promise<number> {
    ...
    const pipelineSeats =
      await this.opportunityApplicationsService.countSeatsInFlight(
        opportunityId,
      );
    return participationSeats + pipelineSeats;
  }

  private readonly liveOpportunityStatuses = [
    'active',
    'live',
    'open',
    'recruiting',
  ...
  private normalizeApplicationStatus(status?: string | null): string | null {
    ...
    if (
      ['rejected', 'not_approved', 'denied', 'declined'].includes(normalized)
    ) {
      return 'rejected';
    }
  ...
  private isParticipationActivelyEnrolled(status?: string | null): boolean {
    ...
    return ['approved', 'verified', 'accepted', 'finalized', 'paid'].includes(
      status,
    );
  }
  ...
  private async findLatestParticipationForProjectAndMemberEmail(
    ...
    const qb = this.participantRepository
      ...
      .andWhere(
        new Brackets((w) => {
          w.where("LOWER(TRIM(COALESCE(p.email, ''))) = :emailNorm", {
            emailNorm,
          });
          if (user?.id) {
  ...
  private async assertTeamMembersNotAlreadySeatedOnOpportunity(
    ...
    for (const member of members) {
      ...
      const row = await this.findLatestParticipationForProjectAndMemberEmail(
        opportunityId,
        emailNorm,
      );
      if (!row) continue;
  ...
  private resolveDisplayNameForUser(user: User): string {
    ...
    const fromOrg =
      user.organization?.contactName != null
        ? String(user.organization.contactName).trim()
        : '';
    return fromOrg || '';
  }
  ...
  private computeStudentProfileCompletion(user: User): {
    ...
    satisfied.push(
      Boolean(typeof user.email === 'string' && user.email.trim()),
    );
    ...
    satisfied.push(
      Boolean(typeof user.department === 'string' && user.department.trim()),
    );
    if (user.requires_cnic) {
      satisfied.push(
        Boolean(typeof user.cnic === 'string' && user.cnic.trim()),
      );
    }
  ...
  private resolveRequiredHoursPerStudent(
    project: Opportunity | null | undefined,
  ): number {
    if (!project) return 0;
    const fromTimeline = this.safeDashboardNumber(
      project.timeline?.expected_hours,
    );
  ...
  private participationTeamBucketKey(
    p: Pick<
      Participation,
      'id' | 'participationMode' | 'teamId' | 'applicationId'
    >,
  ): string {
   ...
   */
  private async participationRowsForStudentProjectTeam(
    app: Participation,
    ...
    if (app.applicationId) {
      const payloadEmails =
        await this.opportunityApplicationsService.findApplyPayloadTeamMemberEmails(
          app.applicationId,
        );
      if (payloadEmails.length > 0) {
        const byEmail = await this.participantRepository
          .createQueryBuilder('p')
          .where('p.projectId = :projectId', { projectId })
          .andWhere("LOWER(TRIM(COALESCE(p.email, ''))) IN (:...emails)", {
   ...
   */
  async getAdminTeamRosterForParticipation(participationId: string): Promise<{
    team_member_count: number;
    team_members: { name: string; email: string; is_team_lead: boolean }[];
  } | null> {
    const app = await this.participantRepository.findOne({
  ...
  private participationToDashboardStatus(
  ...
  private buildCompletedActivityBars(
  ...
  private getApiOpportunityStatus(opportunity: Opportunity): string | null {
    if (opportunity.workflowStage === 'live' && opportunity.admin_approved)
      return 'live';
    if (
  ...
  private getOpportunityEligibility(
    ...
    const userUniversity = this.normalize(
      user.university || user.institution || user.orgName,
    );
    ...
    const deptMatch =
      deptScope === 'all' || (!!userDept && deptSet.includes(userDept));
    ...
    switch (rule) {
      ...
      case 'own_university_only':
        if (!userUniversity || userUniversity !== creatorNorm)
          return ownUniFail();
        return deptMatch ? { eligible: true } : deptFail();
  ...
  async getDashboard(userId: string) {
    ...
    const workingParticipationStatuses = [
      'approved',
      'verified',
      'paid',
      'accepted',
    ...
    const [
      ...
      this.participantRepository.count({
        where: {
          studentId: userId,
          status: In([...workingParticipationStatuses]),
        },
      }),
      this.participantRepository.count({
        where: {
          studentId: userId,
          status: In([...pendingParticipationStatuses]),
        },
      }),
      this.participantRepository.find({
        where: {
          studentId: userId,
          status: In([...participationDashboardStatuses]),
        },
    ...
    const projectIdsForTeams = [
      ...new Set(activeApplications.map((a) => a.projectId)),
    ];
    ...
    if (studentUser) {
      ...
      student_analytics = {
        ...
        verified:
          studentUser.profile_verified === true &&
          studentUser.identity_verified === true,
      };
    } else {
      student_analytics = {
    ...
    const activeProjects = activeApplications.map((app) => {
      ...
      const teamSize =
        teamSizeByKey.get(
          `${app.projectId}|${this.participationTeamBucketKey(app)}`,
        ) ?? 1;
    ...
    const pendingSampleRows = activeApplications
      .filter((app) =>
        (pendingParticipationStatuses as readonly string[]).includes(
          app.status,
        ),
      )
      .slice(0, 3);
    ...
    const reportsUnderReviewSample = reportsUnderReviewList
      .slice(0, 3)
      .map((r) => ({
    ...
    const completedSample =
      latestPaidOrVerified &&
      (latestPaidOrVerified.opportunityId || latestPaidOrVerified.project_id)
        ? {
            id: String(
              latestPaidOrVerified.opportunityId ||
                latestPaidOrVerified.project_id,
            ),
    ...
    const resultsReport = studentReports.find((r) =>
      ['verified', 'paid'].includes(r.status),
    );
    const resultsProjectId = resultsReport
      ? this.reportProjectKey(resultsReport)
      : null;
    ...
    const overview = {
      ...
      impactHistoryBadgeCount: studentReports.filter(
        (r) => r.status !== 'draft',
      ).length,
    ...
    const teamMemberApps = activeApplications.filter(
      (app) =>
        app.participationMode === 'team' &&
        app.isTeamLead !== true &&
        (workingParticipationStatuses as readonly string[]).includes(
          app.status,
        ),
    );
    const teamMemberTasksCount = teamMemberApps.length;
    const teamMemberTasks = teamMemberApps.map((app) => ({
      project_id: String(app.projectId),
      title: app.project?.title || 'Project',
      team_display_name: app.teamDisplayName ?? null,
      href: `/dashboard/student/projects/${app.projectId}/participation`,
    }));
    ...
    const pendingSummary = {
      ...
      items: [
        ...
        ...(teamMemberTasksCount > 0
          ? [
              {
                key: 'student_team_member_tasks',
                title: 'Team attendance',
                count: teamMemberTasksCount,
                href: '/dashboard/student/projects',
                tone: 'warning' as const,
                description:
                  'You were added as a team member — log attendance on your member portal.',
              },
            ]
          : []),
      ],
    ...
    return {
      ...
      data: {
        ...
        pendingSummary,
        team_member_tasks: teamMemberTasks,
        student_analytics,
      },
    };
  }
  ...
  async getOpportunities(query: any, userId?: string) {
    ...
    if (
      !requestedStatus ||
      ['approved', 'active', 'live'].includes(requestedStatus)
    ) {
      ...
      requireAdminApproval =
        this.liveOpportunityStatuses.includes(requestedStatus);
    }
    ...
    if (studentContextId && opportunities.length > 0) {
      ...
      appByOpp =
        await this.opportunityApplicationsService.mapCurrentApplicationsForOpportunities(
          studentContextId,
          opportunityIds,
    ...
    const filtered = opportunities.filter((opportunity) => {
      ...
      const matchesLocation =
        !normalizedLocation ||
        this.normalize(opportunity.location?.city) === normalizedLocation;
      const matchesType =
        !normalizedType ||
        (Array.isArray(opportunity.types) &&
          opportunity.types.some(
            (entry) => this.normalize(entry) === normalizedType,
          ));
        ...
        paginated.map(async (o) => {
          ...
          return {
            ...
            application_stage: app
              ? this.opportunityApplicationsService.applicationStage(
                  app.internalStatus,
                )
              : null,
  ...
  async getStudentProjects(studentId: string) {
    ...
    const pipelineApps =
      await this.opportunityApplicationsService.findNonWithdrawnApplicationsForStudent(
        studentId,
      );
    ...
    const ownNotApplied = createdOpportunities.filter(
    ...
    const studentReports =
      await this.studentReportsService.getMergedReportsForParticipant(
        studentId,
      );
    const reportByProjectId = new Map<string, StudentReport>();
    for (const r of studentReports) {
      const key = String(r.opportunityId || r.project_id || '');
      if (key && !reportByProjectId.has(key)) {
        reportByProjectId.set(key, r);
      }
    }
    ...
    const fromParticipants = await Promise.all(
      applications.map(async (app) => {
        ...
        const base = this.opportunityWorkflow.toStudentProjectCard(
          app.project,
          { teamMembers },
        );
        ...
        const guideRole =
          app.isTeamLead === true
            ...
            : app.participationMode === 'team'
              ...
              : ('individual_owner' as const);
        const phase = resolveParticipationPhase({
          guideRole,
          participationMode: app.participationMode,
          isTeamLead: app.isTeamLead === true,
          emailVerified: app.emailVerified,
          attendanceVerificationPending:
            app.attendanceVerificationRequested === true &&
            !app.adminAttendanceEditable,
          approverType,
        ...
        return {
          ...
          hasApplied: overlay.hasApplied,
          is_team_lead: app.isTeamLead === true,
  ...
  async getProjectById(opportunityId: string, studentUserId?: string) {
    ...
    const isStudentOwner = Boolean(
      studentUserId && opportunity.creatorId === studentUserId,
    );
  ...
  async updateStudentOpportunity(
    ...
    const resubmitSnapshot =
      ...
      opportunity.workflowStage === WORKFLOW_STAGE.REVISION
        ? this.opportunitiesService.snapshotStudentOpportunityResubmit(
            opportunity,
          )
        : null;
    ...
    for (const field of patchableFields) {
      ...
      if (value !== undefined) {
        patch[field as keyof Opportunity] = value;
      }
    }
    ...
    if (resubmitSnapshot) {
      const plan =
        await this.opportunitiesService.applyStudentCreatedOpportunityResubmit(
          opportunity,
          resubmitSnapshot,
        );
    ...
    if (notifyFacultyAfterResubmit) {
      await this.opportunitiesService.notifyFacultyForStudentOpportunityVerification(
        saved,
      );
    }
    if (notifyPartnerAfterResubmit) {
      await this.opportunitiesService.notifyPartnerForStudentOpportunityPartnerQueue(
        saved,
      );
    }
  ...
  async applyToOpportunity(userId: string, dto: ApplyOpportunityDto) {
    ...
    if (
      await this.opportunityApplicationsService.hasOpenPipelineApplication(
        userId,
        dto.opportunityId,
      )
    ) {
    ...
    const latestPipelineApp =
      await this.opportunityApplicationsService.findLatestForStudentOpportunity(
        userId,
        dto.opportunityId,
      );
    const pipelineWasRejected =
      !!latestPipelineApp &&
      this.opportunityApplicationsService.isTerminalRejection(
        latestPipelineApp.internalStatus,
      );
    ...
    if (existingParticipation) {
      ...
      if (existingParticipation.status === 'rejected') {
        ...
        this.isParticipationPendingPipeline(existingParticipation.status)
      ) {
    ...
    const claimedEmailsOnOpenApplications =
      await this.opportunityApplicationsService.collectClaimedEmailsOnOpenApplications(
        dto.opportunityId,
      );
    ...
    const isTeamApply = isTeamApplyFromParticipationAndMembers(
      dto.participation_type,
      dto.team_members,
    );

    const attendanceApproverType = opportunityHasActionablePartnerForAttendance(
      opportunity,
    )
      ? 'partner'
      : 'faculty';
    ...
    if (
      ...
      await this.assertTeamMembersNotAlreadySeatedOnOpportunity(
        dto.opportunityId,
        teamMembersPayload,
      );
    }
    ...
    if (
      ...
      saved.internalStatus === 'pending_faculty'
    ) {
    ...
    return {
      ...
      data: {
        ...
        application_status:
          this.opportunityApplicationsService.toPublicApplicationStatus(
            saved.internalStatus,
          ),
        application_stage: this.opportunityApplicationsService.applicationStage(
          saved.internalStatus,
        ),
  ...
  async withdrawApplication(userId: string, id: string) {
    const pipelineWithdraw = await this.opportunityApplicationsService.withdraw(
      userId,
      id,
    );
    if (pipelineWithdraw) {
  ...
  async getReports(userId: string, organisationId: string) {
    ...
    const reports = activeApplications.map((app) => {
      ...
      if (hoursDone >= requiredHours && requiredHours > 0)
  ...
  private resolveImpactStudentId(
    ...
    if (
      requested &&
      (role === UserRole.SUPER_ADMIN || requested === requestingUserId)
    ) {
      return requested;
    }
  ...
  private hasMeaningfulImpactObjectValue(value: unknown): boolean {
    ...
    return Object.values(value as Record<string, unknown>).some((v) => {
      ...
      if (v && typeof v === 'object')
        return this.hasMeaningfulImpactObjectValue(v);
  ...
  private reportRequiresPartnerApproval(report: StudentReport): boolean {
    return this.reportPartnerApprovalSettings.reportRequiresPartnerApprovalSync(
      report,
      (value) => this.hasMeaningfulImpactObjectValue(value),
    );
  }
  ...
  private isApprovedImpactReport(r: StudentReport): boolean {
    ...
    const hasFinalStatus =
      ...
      (r.admin_status === 'approved' &&
        ['submitted', 'partner_verified'].includes(r.status));
    const partnerApproved =
      !this.reportRequiresPartnerApproval(r) ||
      isReportPartnerStepSatisfied(r.partner_status);
  ...
  private deriveImpactReportPipelineStatus(r: StudentReport): string {
    ...
    if (
      r.status === 'verified' &&
      (r.partner_status !== 'approved' || r.admin_status !== 'approved')
    ) {
      return 'under_review';
    }
    return 'under_review';
  ...
  private getImpactReportHours(report: StudentReport): number {
    ...
    const metricHours = this.toImpactNumber(
  ...
  private async getApprovedOwnedReport(
    ...
    const mayAccess =
      ...
      (await this.participantMayAccessApprovedTeamImpactReport(
        studentId,
        report,
      ));
    if (!mayAccess) {
  ...
  async getImpactHistory(
    ...
    const participationByProject = new Map(
      participations.map((p) => [p.projectId, p]),
    );
    const ownedApprovedReports = reports.filter((r) =>
      this.isApprovedImpactReport(r),
    );
    ...
    for (const p of participations) {
      ...
      if (
        cand &&
        this.isApprovedImpactReport(cand) &&
        !seenApprovedId.has(cand.id)
      ) {
    ...
    const verifiedTimesheets = timesheets.filter(
      (t) => t.status === 'verified',
    );
    ...
    const pendingReports = visibleReports.filter((r) =>
      this.isPendingImpactReport(r),
    );

    const verifiedTimesheetProjectIds = new Set(
      verifiedTimesheets.map((t) => t.opportunityId).filter(Boolean),
    );
    const pendingTimesheetProjectIds = new Set(
      pendingTimesheets.map((t) => t.opportunityId).filter(Boolean),
    );
    const reportHoursForTotals = (
      report: StudentReport,
      coveredProjectIds: Set<string>,
    ) => {
    ...
    const totalHours =
      verifiedTimesheets.reduce((sum, t) => sum + t.hours, 0) +
      reportTotalHours;
    const pendingHours =
      pendingTimesheets.reduce((sum, t) => sum + t.hours, 0) +
      reportPendingHours;
    ...
    const projectIdsFromReports = new Set(
      approvedReports
        .map((r) => this.getReportProjectId(r))
        .filter(Boolean) as string[],
    );
    const projectsCompleted = new Set([
      ...projectIdsFromHours,
      ...projectIdsFromReports,
    ]).size;
    ...
    for (const t of timesheets) {
      ...
      const participationMode =
        part?.participationMode === 'team' ||
        part?.participationMode === 'individual'
          ? part.participationMode
          : null;
      const linkedReport = oppId
        ? latestApprovedReportByOpp.get(oppId)
        : undefined;
      const certFromReport = linkedReport
        ? this.pickCertificateUrlFromReport(linkedReport)
        : null;
      const pdfFromReport = linkedReport
        ? this.pickPdfUrlFromReport(linkedReport)
        : null;
        ...
        actions: {
          certificate_url:
            linkedReport && t.status === 'verified'
              ? (certFromReport ??
                `${basePrefix}/certificates/${linkedReport.id}/download`)
              : null,
          pdf_url:
            linkedReport && t.status === 'verified'
              ? (pdfFromReport ??
                `${basePrefix}/reports/${linkedReport.id}/pdf`)
              : null,
          cii_url:
            linkedReport && t.status === 'verified'
  ...
  /**
   * "My Rankings" — where each of the student's own verified Community Service reports stands
   * on CII against three live-computed cohorts: the same supervising faculty's students, the same
   * university, and the whole CIEL PK network. Mirrors getImpactHistory()'s percentile precedent
   * (live, not a stored "official run" snapshot) and reuses the same verified/CII definitions the
   * Community Award model already uses, so a report counts as "verified" identically everywhere.
   */
  async getCommunityServiceRankings(studentId: string) {
    const MIN_COHORT = 3; // a rank inside a smaller pool ("#1 of 1") is misleading, so it's suppressed

    const allReports = await this.studentReportsRepository.find({
      relations: ['student', 'opportunity'],
    });

    const uniKey = (r: StudentReport): string | null => {
      const raw =
    ...
    return {
      ...
      data: mine.map((r) => {
        ...
        return {
          ...
          title: r.opportunity?.title ?? 'Community Service project',
          cii: scored.cii,
          facultyRank: rankWithin(facultyPool, scored.cii),
