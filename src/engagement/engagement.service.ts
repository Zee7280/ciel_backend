...
import { resolveParticipationForAttendanceUnlock } from './attendance-unlock.util';
...
export class EngagementService {
   ...
   */
  async linkOrphanParticipationsByEmail(
    userId: string,
    ...
    if (n > 0) {
      this.logger.log(
        `Linked ${n} orphan participation row(s) to student ${userId} (${norm})`,
      );
    }
    return n;
  }
  ...
  async preRegister(
    ...
    const normalizedEmailLookup = data.email
      ? this.normalizeParticipantEmail(data.email)
      : '';
    ...
    if (!existing && normalizedEmailLookup) {
      const account = await this.userRepository
        ...
        .where('LOWER(TRIM(user.email)) = :emailNorm', {
          emailNorm: normalizedEmailLookup,
        })
        .getOne();
    ...
    if (existing) {
      ...
      if (!existing.studentId && studentId) {
        existing.studentId = studentId;
        this.logger.log(
          `Linked existing participation ${existing.id} to student ${studentId} during pre-registration`,
        );
      }
    ...
    const participation = this.participantRepository.create(
      participationData as Partial<Participation>,
    );
      ...
      async (manager) => {
        ...
        if (opportunity.status !== 'active' || !opportunity.admin_approved) {
          throw new BadRequestException(
            'This project is not open for applications yet',
          );
        }
        ...
        if (
          ...
          if (
            ...
            existingByStudent.teamId =
              existingByStudent.teamId || existingByTarget.teamId;
        ...
        if (existingByCnic) {
          const cnicRowEmail = (existingByCnic.email || '')
            .toLowerCase()
            .trim();
        ...
        if (
          existingByCnic &&
          existingByCnic.studentId &&
          existingByCnic.studentId !== targetStudentId
        ) {
          this.logger.log(
            `Found CNIC match for student ${existingByCnic.studentId}, will merge with target student ${targetStudentId}`,
          );
        }
        ...
        if (existingByCnic) {
          ...
          if (existingByTarget && existingByTarget.id !== participation.id) {
            ...
            if (sameAccount) {
              this.logger.log(
                `Merging duplicate records for student ${targetStudentId}: KEEPING ${participation.id}, REMOVING ${existingByTarget.id}`,
              );

              participation.applicationId =
                participation.applicationId || existingByTarget.applicationId;
              participation.teamId =
                participation.teamId || existingByTarget.teamId;
              participation.participationMode =
                participation.participationMode ||
                existingByTarget.participationMode;
        ...
        this.logger.log(
          `Participation ${saved.id} successfully PERSISTED within transaction for student ${targetStudentId || 'Guest'}`,
        );
        ...
        if (primaryFacultyEmail || secondaryFacultyEmail) {
          const studentName = targetStudentId
            ? (await manager.findOne(User, { where: { id: targetStudentId } }))
                ?.name || dto.fullName
            : dto.fullName;
          ...
          if (primaryFacultyEmail) {
            try {
              ...
              this.logger.error(
                `Failed to send faculty approval request to ${primaryFacultyEmail}`,
                error.stack,
              );
            }
          }
          ...
          if (secondaryFacultyEmail) {
            try {
              ...
              this.logger.error(
                `Failed to send faculty collaborator notice to ${secondaryFacultyEmail}`,
                error.stack,
              );
            }
          }
  ...
  async getMyParticipants(studentId: string) {
    ...
    const projectIds = Array.from(
      new Set(result.map((p) => p.projectId).filter(Boolean)),
    );
  ...
  async getProjectTeamForReportDossier(projectId: string) {
    ...
    const mapped = participants.map((p) => {
      ...
      const enriched = this.enrichParticipationForReportDossier(
        decrypted,
        participants,
      );
  ...
  async getProjectTeam(projectId: string) {
    ...
    const mapped = await Promise.all(
      participants.map(async (p) => {
        const decrypted = this.decryptParticipation(p);
        const enriched = await this.enrichParticipationForTeamResponse(
          decrypted,
          participants,
        );
        return this.mapParticipationForReportRoster(decrypted, enriched);
  ...
  private dedupeProjectTeamRosterRows(
    ...
    for (const row of rows) {
      ...
      const teamKey = String(
        row.teamId ?? row.team_id ?? row.applicationId ?? 'default',
      );
      ...
      if (em) {
        if (!leadEmailsByTeam.has(teamKey))
          leadEmailsByTeam.set(teamKey, new Set());
        leadEmailsByTeam.get(teamKey)!.add(em);
      }
      if (sid) {
        if (!leadStudentIdsByTeam.has(teamKey))
          leadStudentIdsByTeam.set(teamKey, new Set());
  ...
  private enrichParticipationForReportDossier(
    ...
    return {
      ...
      primaryFacultyEmail:
        participation.primaryFacultyEmail || facultyEmails[0] || null,
      secondaryFacultyEmail:
        participation.secondaryFacultyEmail || facultyEmails[1] || null,
      facultyEmail:
        facultyEmails[0] ||
  ...
  private async enrichParticipationForTeamResponse(
    ...
    return {
      ...
      team_id: participation.teamId || responseTeamId,
      primaryFacultyEmail:
        participation.primaryFacultyEmail || uniqueFacultyEmails[0] || null,
      secondaryFacultyEmail:
        participation.secondaryFacultyEmail || uniqueFacultyEmails[1] || null,
      facultyEmail:
        uniqueFacultyEmails[0] ||
        participation.primaryFacultyEmail ||
        participation.facultySupervisorEmail ||
        null,
      primary_faculty_email:
  ...
  private findFallbackTeamLead(
    participation: Participation,
    teamLeads: Participation[],
  ): Participation | undefined {
    const matchingLead = teamLeads.find(
      (lead) =>
        lead.id !== participation.id &&
        ((Boolean(participation.applicationId) &&
          lead.applicationId === participation.applicationId) ||
          (Boolean(participation.teamId) &&
            lead.teamId === participation.teamId)),
    );
    if (matchingLead) {
      return matchingLead;
    }
  ...
  private resolveResponseTeamId(
    ...
    if (teamLeads.length === 1)
      return (
        teamLeads[0].teamId || teamLeads[0].applicationId || teamLeads[0].id
      );
    return null;
  }
  ...
  async deleteParticipant(requesterUserId: string, participantId: string) {
    ...
    const isSelf =
      (participation.studentId &&
        participation.studentId === requesterUserId) ||
    ...
    if (!isSelf) {
      const requesterParticipation = await this.participantRepository.findOne({
        where: [
          ...
          ...(requesterEmail
            ? [{ projectId: participation.projectId, email: requesterEmail }]
            : []),
        ],
      ...
      if (!sameTeam) {
        throw new ForbiddenException(
          'Only the team lead or the member themselves can remove this seat',
        );
      }
    }
  ...
  private async findParticipationByIdentifier(
    participantId: string,
    relations: string[] = [],
  ): Promise<Participation> {
    this.logger.debug(
      `Searching for participation with identifier: ${participantId}`,
    );
    ...
    if (userRecord) {
      this.logger.debug(
        `Found user matching identifier: ${userRecord.id}. Searching for their project registration...`,
      );
      ...
      if (participationByUser) {
        this.logger.log(
          `Bridged identifier ${participantId} to participation ${participationByUser.id} for student ${userRecord.id}`,
        );
  ...
  private creatorOwnLiveListingAllowsAttendance(
    ...
    if (!opportunity?.creatorId || opportunity.creatorId !== studentId)
      return false;
  ...
  async addAttendanceLog(
    ...
    const hasEvidence =
      Boolean(file) || String(dto.evidenceUploaded) === 'true';
    ...
    if (participation.studentId !== studentId) {
      ...
      const isCnicMatch =
        participation.cnicHash && myCnicHashes.has(participation.cnicHash);

      this.logger.warn(
        `Authorization check: Requester ${studentId} (email: ${uEmail}) vs Participation ${participation.id} (owner: ${participation.studentId}, email: ${pEmail})`,
      );

      if (isEmailMatch || isCnicMatch) {
        const matchType = isEmailMatch ? 'email' : 'CNIC';
        this.logger.log(
          `Auto-claiming participation ${participation.id} for user ${studentId} via transitive ${matchType} match.`,
        );
        ...
        this.logger.log(
          `Participation ${participation.id} auto-claimed by user ${studentId} via transitive ${matchType} matching during attendance logging`,
        );
      } else {
        ...
        if (participation.applicationId) {
          ...
          if (leadRecord) {
            ...
            this.logger.log(
              `Attendance entry by Team Lead ${studentId} authorized for team member record ${participation.id} (applicationId)`,
            );
          }
        }
        ...
        if (!isAuthorizedAsLead && participation.teamId) {
          ...
          if (leadRecord) {
            isAuthorizedAsLead = true;
            this.logger.log(
              `Attendance entry by Team Lead ${studentId} authorized for team member record ${participation.id} (teamId)`,
            );
          }
        }
        ...
        if (!isAuthorizedAsLead) {
          this.logger.error(
            `Not authorized: User ${studentId} (email: ${user?.email}, cnicHash: ${userCnicHash?.slice(-6)}) tried logging attendance for record ${participation.id} (owner: ${participation.studentId}, email: ${participation.email}, cnicHash: ${participation.cnicHash?.slice(-6)})`,
          );
    ...
    const teamPeers = participation.projectId
      ? await this.participantRepository.find({
          where: { projectId: participation.projectId },
        })
      : [];
    const effectiveParticipation =
      resolveParticipationForAttendanceUnlock(participation, teamPeers) ??
      participation;
    ...
    if (
      !creatorLiveBypass &&
      effectiveParticipation.adminAttendanceEditable !== true &&
      !['approved', 'verified', 'accepted', 'finalized'].includes(
        participation.status,
      )
    ) {
      this.logger.warn(
        `Attendance logging attempt for record ${participation.id} in status: ${participation.status}`,
      );
      ...
      const firstLogDate = new Date(
        Math.min(
          ...participation.attendanceLogs.map((l) =>
            new Date(l.dateOfEngagement).getTime(),
          ),
        ),
      );
    ...
    if (file) {
      try {
        evidenceUrl = await this.s3Service.uploadFile(
          file,
          'attendance-evidence',
        );
    ...
    let attendanceApproverType =
      await this.resolveAttendanceApproverTypeForParticipation(
        participation,
        opportunity,
      );
  ...
  private async notifyAttendancePendingReview(
    ...
    const studentEmail = (student?.email || participation.email || '')
      .trim()
      .toLowerCase();
    ...
    if (routing.assignedApproverType === 'faculty') {
      const approver = routing.assignedApproverUserId
        ? await this.userRepository.findOne({
            where: { id: routing.assignedApproverUserId },
          })
        : null;
    ...
    if (routing.assignedApproverType === 'partner') {
      const approver = routing.assignedApproverUserId
        ...
        : null;
      const partnerEmails = this.extractPartnerOfficialEmails(opportunity);
      const recipientEmail = (approver?.email || partnerEmails[0] || '')
        .trim()
        .toLowerCase();
  ...
  async listPendingAttendanceLogs(
    actorUserId: string,
  ...
  private async hydratePendingAttendanceParticipantTeamIds(
    ...
    for (const log of logs) {
      ...
      if (existing) {
        (participant as Participation & { team_id?: string }).team_id =
          existing;
        continue;
      }
  ...
  private resolveTeamIdFromRosterRow(
  ...
  private formatPendingAttendanceResponse(
    logs: AttendanceLog[],
    projectId?: string,
  ) {
    return {
  ...
  private async facultyMemberCanAccessOpportunityForPendingAttendance(
    ...
    const appRepo = this.opportunityRepository.manager.getRepository(
  ...
  async patchAttendanceApproval(
    ...
    actorRole: string | undefined,
    logId: string,
    dto: PatchAttendanceApprovalDto,
  ) {
    ...
    const opportunityCreatorFallback = log.project?.isStudentCreated
      ? null
  ...
  async createAttendanceVerifyRequest(
    ...
    const participant = await this.resolveVerifyRequestTargetParticipant(
      actorUserId,
      projectId,
      dto.participantId,
    );
    if (!participant) {
      throw new NotFoundException(
        'Participation record not found for this project',
      );
    }
    ...
    const canClaimByEmail =
      !participant.studentId && !!actorEmail && participantEmail === actorEmail;
    ...
    if (
      ...
      authorized = this.partnerActorHostsOpportunity(
        actorUserId,
        actor,
        opportunity,
      );
    ...
    if (!authorized) {
      throw new ForbiddenException(
        'Not authorized to request attendance verification',
      );
    }
  ...
  async deleteAttendanceLog(
    ...
    if (participation.studentId !== studentId) {
      throw new ForbiddenException(
        'You can only delete your own attendance entries',
      );
    }
  ...
  async getEngagementMetrics(participantId: string) {
    ...
    const totalHours = logs.reduce(
      (sum, log) => sum + Number(log.sessionHours),
      0,
    );
    ...
    const eis = Math.round(
      hoursScore + continuityScore + spanScore + frequencyScore + evidenceScore,
    );
  ...
  async generateSummary(participantId: string) {
    ...
    if (metrics.evidenceCount > 0) {
      summary +=
        'based on verified attendance continuity and supporting documentation.';
    } else {
  ...
  private getHecDisplay(hours: number): string {
    if (hours >= 48)
      return 'meets Full 3-Credit Equivalent requirements (Extraordinary)';
   ...
   */
  applyNormalizedCnicToParticipation(
    participation: Participation,
    normalizedDigits13: string,
  ): void {
  ...
  private async resolveVerifyRequestTargetParticipant(
    ...
    return this.participantRepository
      ...
      .andWhere("LOWER(TRIM(COALESCE(participant.email, ''))) = :actorEmail", {
        actorEmail,
      })
  ...
  private async resolveFacultyEmailsForAttendanceRouting(
    ...
    if (opportunity.facultyId) {
      const facultyUser = await this.userRepository.findOne({
        where: { id: opportunity.facultyId },
      });
    ...
    const linkageEmails =
      this.resolveOpportunityLinkageFacultyEmails(opportunity);
  ...
  private async resolveTeamIdForRegistration(
    ...
    const direct =
      dtoTeamId || this.normalizeOptionalString(participation.teamId);
    if (direct) return direct;
    ...
    if (participation.applicationId) {
      ...
      const fromLinked = this.readTeamIdFromApplyPayload(
        linkedApp?.applyPayload,
      );
    ...
    if (studentId) {
      const approvedApp = await manager
        ...
        .getOne();
      const fromApp = this.readTeamIdFromApplyPayload(
        approvedApp?.applyPayload,
      );
  ...
  private normalizeAttendanceApproverType(
    value: unknown,
  ): 'faculty' | 'partner' {
  ...
  private resolveOpportunityLinkageFacultyEmails(
    opportunity: Opportunity,
  ): string[] {
    ...
    const rep = (linkage as Record<string, unknown>)
  ...
  private async resolveAttendanceApproverTypeForParticipation(
    ...
    const stored = this.normalizeAttendanceApproverType(
      participation.attendanceApproverType,
    );
    ...
    const effective = resolveEffectiveAttendanceApproverType(
      resolved,
      opportunity,
    );
  ...
  private async findPartnerRoleUserByEmail(
    email: string,
  ): Promise<User | null> {
  ...
  private async findUserIdByPartnerContactEmail(
    email: string,
  ): Promise<string | null> {
    const normalized = email.trim().toLowerCase();
    if (!normalized) return null;
  ...
  private async resolvePartnerOwnerUserId(
    opportunity: Opportunity,
  ): Promise<string | null> {
    const partnerEmails = this.extractPartnerOfficialEmails(opportunity);
    ...
    if (opportunity.organizationId) {
      ...
      const orgMemberUser = await this.userRepository
        ...
        .andWhere('user.role != :studentRole', {
  ...
  private async resolveApplicationFacultyEmails(
    participation: Participation,
  ): Promise<string[]> {
    let app: OpportunityApplication | null = null;
    ...
    return this.normalizeEmailList([
      ...
      typeof payload.primary_faculty_email === 'string'
        ? payload.primary_faculty_email
        : null,
      typeof payload.secondary_faculty_email === 'string'
        ? payload.secondary_faculty_email
        : null,
    ]);
  }
   ...
   */
  async reconcilePartnerAttendanceAssigneeForOpportunity(
    opportunity: Opportunity,
  ): Promise<void> {
   ...
   */
  async reconcileMisroutedPartnerAttendanceForOpportunity(
    opportunity: Opportunity,
  ): Promise<void> {
    if (
      !opportunity.isStudentCreated ||
