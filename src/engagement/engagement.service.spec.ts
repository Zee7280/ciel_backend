...
describe('EngagementService', () => {
  ...
  describe('getEngagementMetrics', () => {
    it('should calculate metrics correctly', async () => {
      const mockLogs: any[] = [
        ...
        {
          dateOfEngagement: '2023-10-02',
          sessionHours: 4,
          evidenceUploaded: true,
        },
        {
          dateOfEngagement: '2023-10-08',
          sessionHours: 4,
  ...
  describe('getProjectTeam', () => {
    ...
    it('should drop ghost duplicate member rows that share the team lead email', async () => {
      ...
      expect(result[0]).toEqual(
        expect.objectContaining({ id: t.id.lead, isTeamLead: true }),
      );
    });
  ...
  describe('EIS Engine Logic', () => {
    it('should give 100 for perfect engagement', async () => {
      ...
      for (let i = 0; i < 16; i++) {
        ...
        mockLogs.push({
          dateOfEngagement: date1.toISOString().split('T')[0],
          sessionHours: 1.5,
          evidenceUploaded: true,
        });
        mockLogs.push({
          dateOfEngagement: date2.toISOString().split('T')[0],
          sessionHours: 1.5,
          evidenceUploaded: true,
        });
      }
  ...
  describe('addAttendanceLog', () => {
    it('should create and save an attendance log', async () => {
      ...
      const result = await service.addAttendanceLog(
        t.id.u1,
        t.id.participation,
        dto,
      );
    ...
    it('should throw error if session exceeds 9 hours', async () => {
      ...
      await expect(service.addAttendanceLog('u1', 'p1', dto)).rejects.toThrow(
        'Daily attendance cannot exceed 9 hours',
      );
    });
    ...
    it('should throw error if same-day total would exceed 9 hours', async () => {
      const mockParticipation = {
        ...
        studentId: 'u1',
        status: 'approved',
        attendanceLogs: [{ dateOfEngagement: '2023-10-01', sessionHours: 6 }],
      };
      const dto = {
        dateOfEngagement: '2023-10-01',
        startTime: '14:00',
    ...
    it('should allow a session that brings same-day total to exactly 9 hours', async () => {
      ...
      const mockParticipation = {
        ...
        status: 'approved',
        primaryFacultyEmail: t.email.faculty,
        attendanceLogs: [{ dateOfEngagement: '2023-10-01', sessionHours: 5 }],
      };
      const dto = {
        dateOfEngagement: '2023-10-01',
        startTime: '14:00',
  ...
  describe('createAttendanceVerifyRequest', () => {
    it('should reject non-privileged user targeting another participant by participantId', async () => {
      ...
      await expect(
        service.createAttendanceVerifyRequest(
          t.id.student,
          'student',
          t.id.project,
          dto,
    ...
    it('should reject faculty user with no project linkage targeting another participant', async () => {
      ...
      (mockParticipationRepository as any).createQueryBuilder = jest
        .fn()
        .mockReturnValue({
          where: jest.fn().mockReturnThis(),
          andWhere: jest.fn().mockReturnThis(),
    ...
    it('should allow non-privileged user when unclaimed participant email matches actor', async () => {
      ...
      const result = await service.createAttendanceVerifyRequest(
        t.id.student,
        'student',
        t.id.project,
        dto,
    ...
    it('should OVERRIDE if the email matches', async () => {
      ...
      const mockManager = {
        ...
        createQueryBuilder: jest
          ...
          .mockReturnValueOnce(mockUserQueryBuilder(null))
          .mockReturnValueOnce(
            mockParticipationQueryBuilder(
              existingParticipation as Participation,
            ),
          ),
        create: jest.fn(),
