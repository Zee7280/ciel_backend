import type { ReportCIIauditMeta } from './parse-cii-audit-summary.util';
...
export function isSection11V61Evaluation(
  value: unknown,
): value is Section11V61Evaluation {
...
function redFlagMagnitudeThresholds(version: string): {
  critical: number;
  moderate: number;
} {
...
function collectSectionRedFlags(
  evalData: Section11V61Evaluation,
): Section11RedFlag[] {
...
function formatScoreBreakdownBlock(
  breakdown: Section11ScoreBreakdown,
): string | null {
  ...
  for (let n = 1; n <= 10; n++) {
    ...
    lines.push(
      `Section ${n}: ${raw}/${max} → weighted ${row.weighted_contribution}`,
    );
  }

  if (typeof breakdown.subtotal_before_bonuses_penalties === 'number') {
    lines.push(
      `Subtotal (sections): ${breakdown.subtotal_before_bonuses_penalties}`,
    );
  }
  ...
  if (itemised.length) {
    ...
    for (const flag of itemised) {
      ...
      const deduction =
        typeof flag.deduction === 'number' ? ` (${flag.deduction})` : '';
      lines.push(
        `  ${code}${flag.description || 'Red flag'}${deduction}`.trim(),
      );
    }
  }

  if (
    typeof breakdown.red_flag_total === 'number' &&
    breakdown.red_flag_total !== 0
  ) {
    lines.push(
...
function redFlagCapWasApplied(evalData: Section11V61Evaluation): boolean {
  return Boolean(
    evalData.penalties?.red_flag_cap_applied ||
    evalData.score_breakdown?.red_flag_cap_applied,
  );
}

function bucketRedFlags(
  flags: Section11RedFlag[],
  version: string,
): {
  ...
  const { critical: criticalAt, moderate: moderateAt } =
    redFlagMagnitudeThresholds(version);
  ...
  for (const flag of flags) {
    ...
    const magnitude = Math.abs(
      typeof flag.deduction === 'number' ? flag.deduction : 0,
    );
...
function deriveRecommendedAction(evalData: Section11V61Evaluation): string {
  ...
  if (level <= 1 || evalData.hec_compliant === false)
  ...
  const redFlagTotal =
    evalData.penalties?.red_flag_total ??
    evalData.score_breakdown?.red_flag_total;
  const cap = redFlagCapForVersion(version);
  if (typeof redFlagTotal === 'number' && redFlagTotal <= -(cap * 0.8))
    return 'Partial Deduction Recommended';
  if (typeof redFlagTotal === 'number' && redFlagTotal <= -(cap * 0.4))
    return 'Accepted with Observation';
...
function formatConcerns(concerns: unknown[] | undefined): {
  ...
  for (const item of concerns) {
    ...
    if (item && typeof item === 'object') {
      ...
      const text = String(
        row.message || row.concern || row.rationale || '',
      ).trim();
      if (!text) continue;
      ...
      if (sev.includes('critical') || sev.includes('severe'))
        ...
        sev.includes('material') ||
        sev.includes('significant')
      ) {
        moderate.push(text);
 ...
 */
export function formatSection11V61AsSummaryText(
  evalData: Section11V61Evaluation,
): string {
  ...
  for (let n = 1; n <= 10; n++) {
    ...
    const scoreLine =
      scoreRow &&
      typeof scoreRow.raw === 'number' &&
      typeof scoreRow.max === 'number'
  ...
  const lift = (evalData.narrative?.lift_criteria || [])
    .map((c) => String(c || '').trim())
    .filter(Boolean);
  if (lift.length) {
    parts.push(
...
export function section11V61ToAuditMeta(
  evalData: Section11V61Evaluation,
): ReportCIIauditMeta {
  const version = evaluationVersionLabel(evalData);
  const concernBuckets = formatConcerns(evalData.narrative?.concerns);
  const redFlagBuckets = bucketRedFlags(
    collectSectionRedFlags(evalData),
    version,
  );
  const lift = (evalData.narrative?.lift_criteria || [])
    .map((c) => String(c || '').trim())
  ...
  const level = evalData.level ?? 0;
  const cii =
    typeof evalData.cii === 'number' ? Math.round(evalData.cii) : null;
  const recommended = deriveRecommendedAction(evalData);
  const redFlagTotal = resolveRedFlagTotal(evalData);
  ...
  const credibility =
    [iasNote, eisNote, redFlagNote].filter(Boolean).join(' · ') || null;
  ...
  return {
    critical_red_flags: mergeText(
      redFlagBuckets.critical,
      concernBuckets.critical,
    ),
    moderate_issues: mergeText(
      redFlagBuckets.moderate,
      concernBuckets.moderate,
    ),
    ...
    needs_revision:
      level <= 1 ||
      evalData.hec_compliant === false ||
      recommended === 'Revision Requested',
  };
}
