...
export function isSection11V81Evaluation(
  value: unknown,
): value is Section11V81Evaluation {
  ...
  if (version === 'v8.2' || version === 'v8.1' || version.startsWith('v8.'))
    return true;
  if (v.final_result && typeof v.final_result.cii_score === 'number')
    return true;
  if (
    Array.isArray(v.section_scores) &&
    v.section_scores.length > 0 &&
    v.student_feedback
  )
    return true;
  return false;
...
function resolveObtainedScore(evalData: Section11V81Evaluation): number {
  ...
  return Math.min(
    CII_PLATFORM_MAX,
    Math.round(sumSectionScores(evalData) * 10) / 10,
  );
}
...
function resolveCiiScore(evalData: Section11V81Evaluation): number | null {
  ...
  const trace = asRecord(evalData.cii_calculation_trace);
  const fromTraceFinal =
    pickNumber(trace.cii_final) ?? pickNumber(trace.cii_clamped);
...
function deriveRecommendedAction(evalData: Section11V81Evaluation): string {
  ...
  const admin = pickString(
    resolveAdminDiagnostics(evalData)?.approval_recommendation,
  );
  if (evalData.final_result?.resubmission_required)
    return 'Resubmission Required';
  if (evalData.final_result?.admin_review_required)
...
export function formatSection11V81AsSummaryText(
  evalData: Section11V81Evaluation,
): string {
  ...
  const summaryText =
    'summary' in (feedback || {})
      ...
      : pickString(
          (feedback as { why_score_is_high_or_low?: string })
  ...
  if (
    Array.isArray(evalData.overall_strengths) &&
    evalData.overall_strengths.length
  ) {
    parts.push(
  ...
  if (
    Array.isArray(evalData.overall_limitations) &&
    evalData.overall_limitations.length
  ) {
    parts.push(
  ...
  const sections = Array.isArray(evalData.section_scores)
    ? evalData.section_scores
    : [];
  for (const row of sections) {
    const n = row.section_number;
    if (!n || n < 1 || n > 10) continue;
    const title =
      pickString(row.section_name) || SECTION_TITLES[n] || `Section ${n}`;
    const sectionScore = resolveSectionScore(row);
    const score =
      sectionScore !== null ? `${sectionScore}/${row.weight ?? '—'}` : '—';
    const body = [
      row.what_was_done,
      row.appreciation,
      row.strengths,
  ...
  if (actions.length) {
    parts.push(
      `Component 8 — Top 5 Improvement Actions\n${actions.map((a, i) => `${i + 1}. ${a}`).join('\n')}`,
    );
  }
  ...
  parts.push(
    [
      ...
      cii !== null
        ? `Current Band: ${badgeTitle || '—'} — score: ${Math.round(cii)} / ${resolveCiiScoreMax(evalData)}`
        : null,
...
export function section11V81ToAuditMeta(
  evalData: Section11V81Evaluation,
): ReportCIIauditMeta {
  const redFlagBuckets = bucketRedFlags(
    Array.isArray(evalData.red_flags) ? evalData.red_flags : [],
  );
  const feedback = resolveStudentFeedback(evalData);
  const actions = (
    (feedback as { five_specific_actions?: string[] })?.five_specific_actions ||
  ...
  return {
    ...
    final_remark: [
      cii !== null
        ? `CII: ${Math.round(cii)}/${resolveCiiScoreMax(evalData)}`
        : null,
      evalData.final_result?.badge_title
        ? `${evalData.final_result.badge_title}`
        : null,
    ...
    needs_revision:
      ...
      recommended === 'Resubmission Required',
  };
}
...
export function buildCiiSnapshotFromV81(evalData: Section11V81Evaluation) {
  ...
  return {
    totalScore: rounded,
    level:
      pickString(evalData.final_result?.badge_title) ||
      `Level ${evalData.final_result?.level ?? ''}`,
