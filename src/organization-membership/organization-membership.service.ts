...
export type MembershipFeeSubmissionUi =
  | 'none'
  | 'pending_review'
  | 'approved'
  | 'rejected';
...
export class OrganizationMembershipService {
  ...
  async getUiFlags(user: User): Promise<{
    ...
    const approvedSubmission = await this.latestSubmissionForUserOrOrganization(
      user,
      'approved',
    );
    if (approvedSubmission) {
      if (user.status === 'pending_membership_payment') {
        await this.activateMembershipAccounts(
          user.id,
          user.organization?.id ?? approvedSubmission.organizationId,
        );
        user.status = 'active';
      }
      return {
  ...
  private latestSubmissionForUserOrOrganization(
    ...
    const qb = this.feeRepo.createQueryBuilder('fee').where(
      new Brackets((sub) => {
        sub.where('fee.userId = :userId', { userId: user.id });
        if (organizationId) {
          sub.orWhere('fee.organizationId = :organizationId', {
            organizationId,
          });
        }
      }),
  ...
  private async activateMembershipAccounts(
    userId: string,
    organizationId?: string | null,
  ) {
  ...
  async submitProof(
    ...
    if (!(await this.roleRequiresMembershipPayment(user.role))) {
      throw new BadRequestException(
        'Membership fee does not apply to this account',
      );
    }
    if (user.status !== 'pending_membership_payment') {
      throw new BadRequestException(
        'No membership payment is required for this account',
      );
    }
    ...
    return {
      success: true,
      message:
        'Payment proof submitted. An administrator will review it shortly.',
    };
  }
  ...
  private formatAdminMembershipRow(
    r: OrganizationMembershipFee,
  ): AdminMembershipFeeRowDto {
    const u = r.user;
