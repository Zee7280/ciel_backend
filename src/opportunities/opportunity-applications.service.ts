...
import {
  ...
  OpportunityApplicationInternalStatus,
} from './entities/opportunity-application.entity';
...
export class OpportunityApplicationsService {
   ...
   */
  private facultyJoinApplicationTeamMembersForDisplay(
    ...
    const payload =
      app.applyPayload && typeof app.applyPayload === 'object'
        ...
        : {};
    const ptRaw =
      typeof payload['participation_type'] === 'string'
        ? payload['participation_type'].trim()
        : '';
    ...
    const raw = Array.isArray(payload['team_members'])
      ...
      : [];
    if (
      isTeamApplyFromParticipationAndMembers(payload['participation_type'], raw)
    ) {
  ...
  adminBrowseApplicationTeamSummaryForQueue(app: OpportunityApplication): {
    ...
    const { participation_type, team_members } =
      this.facultyJoinApplicationTeamMembersForDisplay(app);
   ...
   */
  async collectClaimedEmailsOnOpenApplications(
  ...
  async isTeamSlugInUseOnOpportunity(
    ...
    const inflight = await this.appRepo
      ...
      .andWhere(`TRIM(COALESCE(a.apply_payload->>'team_id','')) = :teamId`, {
        teamId,
      })
      .getCount();
    return inflight > 0;
  ...
  private async findReportsForOpportunityAndStudents(
    opportunityId: string,
    studentIds: string[],
  ) {
  ...
  private participationMobileAndCnicSnapshot(member: Participation) {
    const decrypted = member.cnic
      ? this.engagementService.decryptCnicInternal(member.cnic)
      : '';
  ...
  private parsePendingRosterSyntheticTeammateId(
    memberId: string,
  ): { applicationId: string; emailNormalized: string } | null {
  ...
  private ensurePendingApplyPayloadMatchesTeam(
  ...
  private async adminPatchPendingSyntheticTeammate(
    ...
    if (!app || !PENDING_PIPELINE.includes(app.internalStatus)) {
      throw new NotFoundException(
        'Pending application roster entry not found for this opportunity',
      );
    }
    ...
    if (leadNorm === this.normalizeEmail(emailNormalized)) {
      throw new BadRequestException(
        'Lead row is patched with the application member id — use Update on the lead line instead.',
      );
    }

    const teamMembersRaw = Array.isArray(payloadCopy['team_members'])
      ? ([
          ...(payloadCopy['team_members'] as Array<Record<string, unknown>>),
    ...
    const ix = teamMembersRaw.findIndex(
      (row) =>
        ...
        this.normalizeEmail(row['email']) ===
          this.normalizeEmail(emailNormalized),
    );
    if (ix < 0) {
      throw new NotFoundException(
        'That teammate email is not on this application roster',
      );
    }
    const row = { ...teamMembersRaw[ix] };
    ...
    const mateNorm = mateEmailRaw
      ? this.normalizeEmail(mateEmailRaw)
      : this.normalizeEmail(emailNormalized);
    ...
    if (linked) {
      ...
      if (dto.university_name?.trim())
        patch.university = dto.university_name.trim();
      if (dto.academic_program?.trim())
        patch.major = dto.academic_program.trim();
      if (dto.department?.trim()) patch.department = dto.department.trim();
      if (Object.keys(patch).length)
        await this.usersService.update(linked.id, patch);
    }
    ...
    const rawCnic =
      typeof row.admin_cnic === 'string' && row.admin_cnic.trim()
        ? row.admin_cnic.trim()
        : '';
    const phoneMate = prof?.phone?.trim().length
  ...
  private async adminPatchPendingApplicationLead(
    ...
    if (dto.cnic?.trim()) {
      ...
      const hash =
        this.engagementService.getCnicHashForNormalizedDigits(
          normalizedCnicDigits,
        );
    ...
    if (syncLinkedUserProfile && app.studentUserId) {
      ...
      if (dto.university_name?.trim())
        uPatch.university = dto.university_name.trim();
      if (dto.academic_program?.trim())
        uPatch.major = dto.academic_program.trim();
    ...
    const phoneFromApply =
      typeof mergedPayload['contact_phone_e164'] === 'string'
        ? mergedPayload['contact_phone_e164'].trim()
        : '';
    const phoneFromUser = leadProfile?.phone?.trim().length
    ...
    const adminCorr =
      ...
      !Array.isArray(mergedPayload['admin_correction'])
        ...
        : {};
    const yearFromCorr =
      typeof adminCorr.year_of_study === 'string' &&
    ...
    const memberPayload: Record<string, unknown> = {
      ...
      university_id: null,
      university_name:
        leadProfile?.university ?? leadProfile?.institution ?? null,
  ...
  private pickLatestReportForStudent(
  ...
  private attendanceLogNeedsPartnerOrFacultyReview(
    log: AttendanceLog,
  ): boolean {
  ...
  private async rollupAttendanceReviewsForParticipants(
    ...
    Map<string, { sessions_total: number; sessions_pending_review: number }>
  > {
    const out = new Map<
      string,
      { sessions_total: number; sessions_pending_review: number }
    >();
    const dedup = [
      ...new Set(participantIds.filter((id) => Boolean(id?.trim()))),
    ];
    ...
    for (const id of dedup) {
      ...
      out.set(
        ...
        arr
          .slice(0, cap)
          .map((log) =>
            this.serializeAttendanceSessionForTracker(log, opportunity),
          ),
      );
    }
    return out;
  ...
  private trackerJoinApplicationsStageLabel(
  ...
  private trackerImpactReportEnrollmentFields(
  ...
  private isLowProgressReportStatus(
  ...
  private isBlockingReportStatusForTeamMerge(
    status: string | null | undefined,
  ): boolean {
    const st = String(status || '')
      .trim()
   ...
   */
  async adminMergeOpportunityTeamMembers(
    opportunityId: string,
    dto: AdminMergeTeamMembersDto,
  ) {
    const memberIds = [
      ...new Set(
        dto.member_participation_ids.map((id) => id.trim()).filter(Boolean),
      ),
    ];
    ...
    if (memberIds.length < 2) {
      throw new BadRequestException(
        'Select at least two enrollments to merge.',
      );
    }
    if (!memberIds.includes(leadId)) {
      throw new BadRequestException(
        'lead_participation_id must be included in member_participation_ids.',
      );
    }
    ...
    const mergeDisplayName = buildTeamDisplayName(opportunity, leadRow);
    ...
    await this.participationRepo.manager.transaction(async (em) => {
      ...
      for (const row of members) {
        ...
        row.teamDisplayName = mergeDisplayName;
        row.formationSource = row.formationSource || 'admin_merge';
  ...
  async adminListOpportunityTeams(
    opportunityId: string,
    ...
    const rows = await this.participationRepo
      ...
      .andWhere('p.status IN (:...statuses)', {
        statuses: [...TEAM_ACTIVE_PARTICIPATION_STATUSES],
      })
    ...
    for (const row of rows) {
      ...
      const groupId =
        normalizedTeamId || `individual:${row.studentId || row.id}`;
    ...
    const studentIds = rows
      .map((r) => r.studentId)
      .filter((id): id is string => Boolean(id));
    const reports = await this.findReportsForOpportunityAndStudents(
      opportunityId,
      studentIds,
    );
    ...
    for (const [groupId, members] of rowsByTeamId.entries()) {
      ...
      if (!isIndividualEntry && actualTeamId) {
        ...
        if (leadCount > 1) {
          await this.reconcileDuplicateTeamLeads(
            opportunityId,
            actualTeamId,
            canonicalLead.id,
          );
      ...
      const teamReportStatus = this.aggregateTeamReportStatus(
        memberPayload.map((m) => m.report_status),
      );
      ...
      const fallbackTeamName =
        lead?.student?.name ?? lead?.fullName ?? 'Individual Participant';
      const teamDisplayName =
        actualTeamId && lead
          ? lead.teamDisplayName ||
            members.find((m) => (m.teamDisplayName || '').trim())
              ?.teamDisplayName ||
            `Team ${actualTeamId}`
          : fallbackTeamName;
      data.push({
        ...
        team_display_name: actualTeamId ? teamDisplayName : null,
    ...
    for (const app of pendingPipelineApps) {
      ...
      for (const m of teamMembersRaw) {
        const em =
          typeof m?.email === 'string' ? this.normalizeEmail(m.email) : '';
    ...
    for (const app of pendingPipelineApps) {
      ...
      const leadProfile = leadEmail
        ? pendingUserByEmail.get(leadEmail)
        : undefined;
      ...
      if (!isTeamApply) {
        ...
        const phoneFromApplyIndividual =
          typeof payload['contact_phone_e164'] === 'string'
            ...
            : '';
        const phoneFromUserIndividual = leadProfile?.phone?.trim().length
          ? `${(leadProfile.countryCode || '').trim()}${leadProfile.phone.trim()}`.trim()
          : '';
          ...
          members: [
            {
              ...
              report_available: false,
              phone_number:
                phoneFromApplyIndividual || phoneFromUserIndividual || null,
              cnic_display: leadProfile?.cnic
                ? this.formatPakCnicDigitsDisplay(
                    leadProfile.cnic.replace(/\D/g, ''),
                  )
                : null,
              university_id: null,
              university_name:
    ...
    if (opp) {
      await this.engagementService.reconcileMisroutedPartnerAttendanceForOpportunity(
        opp,
      );
      await this.engagementService.reconcilePartnerAttendanceAssigneeForOpportunity(
        opp,
      );
    }
    ...
    for (const r of pipelineCountRows) {
      ...
      if (key in appsByStatus)
        appsByStatus[key] = Number((r as { cnt?: unknown }).cnt) || 0;
    }
    const awaitingSide =
      opp?.faculty_verification_status === 'pending_faculty'
        ...
        : opp &&
            ...
            opp.execution_verification_status === 'pending_execution'
    ...
    const participantPkIds = [
      ...new Set(
        rows.map((pr) => pr.id).filter((id): id is string => Boolean(id)),
      ),
    ];
    const attendanceRollupByParticipant =
      participantPkIds.length > 0
        ? await this.rollupAttendanceReviewsForParticipants(
            opportunityId,
            participantPkIds,
          )
        : new Map<
    ...
    const attendancePreviewByParticipant =
      participantPkIds.length > 0
        ? await this.attendanceSessionsPreviewByParticipant(
            opportunityId,
            participantPkIds,
            35,
            opp,
    ...
    const applications_roster = allApplications.map((a) => {
      const { participation_type, team_members } =
        this.facultyJoinApplicationTeamMembersForDisplay(a);
      let enrollment_tracking: Record<string, unknown> | null = null;

      const seatRow =
        participationByApplicationId.get(a.id) ??
        (a.studentUserId
          ? participationByStudentUserIdFirst.get(a.studentUserId)
          : undefined);
      ...
      return {
        ...
        application_status: this.toPublicApplicationStatus(
          a.internalStatus,
          seatRow,
        ),
        application_stage: this.applicationStage(a.internalStatus),
        join_stage_display: this.trackerJoinApplicationsStageLabel(
          a.internalStatus,
        ),
        participation_type,
        team_members,
    ...
    const execOrg =
      opp?.executing_organization &&
      typeof opp.executing_organization === 'object'
    ...
    return {
      summary: {
          ...
          ? {
              ...
              executing_organization_name:
                typeof execOrg?.name === 'string' && execOrg.name.trim()
                  ? execOrg.name.trim()
                  : typeof execOrg?.organization_name === 'string' &&
                      execOrg.organization_name.trim()
                    ? execOrg.organization_name.trim()
                    : null,
              executing_organization_email:
                typeof execOrg?.official_email === 'string' &&
              ...
              attendance_routing_override:
                opp.attendanceRoutingOverride ?? 'auto',
        ...
        applications_pipeline_total_non_withdrawn: Object.values(
          appsByStatus,
        ).reduce((s, n) => s + n, 0),
        applications_roster,
      },
  ...
  async adminDeleteOpportunityTeam(opportunityId: string, teamId: string) {
    await this.appRepo.manager.transaction(async (em) => {
      const members = await this.findActiveParticipationsForAdminTeamRoute(
        opportunityId,
        teamId,
        em,
      );
  ...
  private async salvageTeammateSeatsFromDuplicateRows(
    ...
    const leadUser = await this.userRepo.findOne({
   ...
   */
  async adminDedupeStudentParticipationSeats(
    opportunityId: string,
    ...
    if (!seats.length) {
      throw new NotFoundException(
        'No active enrollment seats found for this student on the project',
      );
    }
    if (seats.length === 1) {
      return {
    ...
    const ranked = [...seats].sort((a, b) => {
      ...
      if (Boolean(b.applicationId) !== Boolean(a.applicationId)) {
        return b.applicationId ? 1 : -1;
      }
      if (Boolean(b.isTeamLead) !== Boolean(a.isTeamLead)) {
        return b.isTeamLead ? 1 : -1;
      }
   ...
   */
  private async salvageAllDuplicateAccountSeatsWithoutDelete(
    opportunityId: string,
  ): Promise<number> {
    ...
    for (const [studentId, seats] of byStudent.entries()) {
      ...
      for (const seat of seats) {
        attendanceCounts.set(
          seat.id,
          await this.attendanceLogRepo.count({
            where: { participantId: seat.id },
          }),
        );
      }
  ...
  async adminHealOpportunityTeamEnrollments(opportunityId: string) {
    ...
    summary.report_members_restored =
      await this.provisionMissingMembersFromReportSection1(opportunityId);
    ...
    for (const [studentId, seats] of byStudent.entries()) {
      ...
      summary.duplicate_student_seats_removed +=
        Number(
          (result as { data?: { removed_count?: number } }).data?.removed_count,
        ) || 0;
    }
  ...
  private async rankParticipationSeatsForHeal(
    seats: Participation[],
  ): Promise<Participation[]> {
    const ranked = await Promise.all(
      seats.map(async (seat) => {
        const owner = await this.userRepo
          .createQueryBuilder('u')
          .where('LOWER(TRIM(u.email)) = :em', {
            em: this.normalizeEmail(seat.email),
          })
          .getOne();
        return {
          seat,
  ...
  private async moveAttendanceLogsToParticipant(
    fromId: string,
    toId: string,
  ): Promise<void> {
  ...
  private async provisionMissingMembersFromReportSection1(
    opportunityId: string,
  ): Promise<number> {
    ...
    const leadSeat =
      (await findCanonicalTeamLeadParticipation(
        this.participationRepo,
        opportunityId,
        {},
      )) ??
    ...
    const teamId =
      (leadSeat?.teamId || '').trim() || this.generateAdminTeamId();
  ...
  async adminDeleteOpportunityTeamMember(
    opportunityId: string,
    teamId: string,
    memberId: string,
  ) {
    ...
    await this.appRepo.manager.transaction(async (em) => {
      ...
      const remainingTeamMembers = dbTeamId
        ...
        : [];
      if (
        remainingTeamMembers.length &&
        !remainingTeamMembers.some((p) => p.isTeamLead)
      ) {
  ...
  async adminPatchOpportunityTeamMember(
    ...
    const pendingSynth =
      this.parsePendingRosterSyntheticTeammateId(decodedMemberId);
    if (pendingSynth) {
    ...
    if (!participation) {
      ...
      if (this.looksLikeUuidParam(decodedMemberId)) {
        ...
        if (pendingLead) {
          return this.adminPatchPendingApplicationLead(
            pendingLead,
            teamIdParam,
            dto,
            decodedMemberId,
      ...
      throw new NotFoundException(
        'Team member enrollment not found for this opportunity',
      );
    }
    ...
    if (dto.cnic?.trim()) {
      this.engagementService.applyNormalizedCnicToParticipation(
        participation,
        dto.cnic,
      );
      const digitsUser = dto.cnic.replace(/\D/g, '');
      if (
        syncLinkedUserProfile &&
        participation.studentId &&
        digitsUser.length === 13
      ) {
        await this.usersService.update(participation.studentId, {
          cnic: digitsUser,
        });
    ...
    if (dto.academic_program?.trim()) {
      ...
      if (syncLinkedUserProfile && participation.studentId) {
        await this.usersService.update(participation.studentId, {
          major: dto.academic_program.trim(),
        });
      }
    }
    ...
    let reportStatusRet: 'not_started' | 'in_progress' | 'completed' =
      'not_started';
    if (p.studentId) {
      const reports = await this.findReportsForOpportunityAndStudents(
        opportunityId,
    ...
    const mode = String(p.participationMode ?? '')
      .trim()
      .toLowerCase();
    const isIndividualSeat =
      !(p.teamId || '').trim().length || mode === 'individual';
  ...
  async adminListIncompleteReportApplicants(opportunityId: string) {
    ...
    for (const [studentUserId, row] of byStudent) {
      ...
      if (
        rep &&
        REPORT_STATUSES_EXCLUDED_FROM_INCOMPLETE_LIST.includes(rep.status)
      )
        continue;
      data.push({
        ...
        report_status: rep
          ? this.mapReportStatusForAdminList(rep.status)
          : 'not_started',
      });
    }
   ...
   */
  async adminDeleteApprovedApplicationForIncompleteReport(
    opportunityId: string,
    applicationId: string,
  ) {
    await this.appRepo.manager.transaction(async (em) => {
      ...
      if (!app) {
        throw new NotFoundException(
          'Application not found for this opportunity',
        );
      }
      ...
      for (const sid of studentIds) {
        ...
        if (
          forSid.some((r) =>
            REPORT_STATUSES_BLOCKING_ADMIN_SEAT_REMOVAL.includes(r.status),
          )
        ) {
          throw new BadRequestException(
  ...
  async findLatestForStudentOpportunity(
    studentUserId: string,
    opportunityId: string,
  ) {
    const rows = await this.appRepo.find({
  ...
  private static readonly LIVE_LISTING_STATUSES = [
    'active',
    'live',
    'open',
    'recruiting',
  ...
  isCreatorOwnLiveListing(
    opportunity: Pick<
      Opportunity,
      'creatorId' | 'admin_approved' | 'workflowStage' | 'status'
    >,
    studentUserId: string,
  ): boolean {
    if (!opportunity.creatorId || opportunity.creatorId !== studentUserId)
      return false;
  ...
  async resolveStudentJoinOverlay(
    ...
    opportunity: Pick<
      Opportunity,
      'id' | 'creatorId' | 'admin_approved' | 'workflowStage' | 'status'
    >,
    ...
    if (ownLive) {
      const pending = rows.find((r) =>
        PENDING_PIPELINE.includes(r.internalStatus),
      );
      if (pending) {
        return {
          app: pending,
          applicationStatus: this.toPublicApplicationStatus(
            pending.internalStatus,
            participation,
          ),
      ...
      return {
        ...
        applicationStage: approvedApp
          ? this.applicationStage(approvedApp.internalStatus)
          : null,
    ...
    if (participation) {
      ...
      if (
        ...
        ['rejected', 'not_approved', 'denied', 'declined'].includes(st)
      ) {
  ...
  async hasOpenPipelineApplication(
    studentUserId: string,
    opportunityId: string,
  ): Promise<boolean> {
  ...
  async countPendingAdmin(): Promise<number> {
    return this.appRepo.count({
      where: {
        internalStatus: In(['pending_admin', 'pending_partner']),
        withdrawnAt: IsNull(),
      },
    });
  }
  ...
  async createApplication(params: {
    ...
    const primary = params.primaryFacultyEmail
      ? this.normalizeEmail(params.primaryFacultyEmail)
      : null;
    ...
    const attendanceApproverType =
      params.attendanceApproverType === 'partner' ? 'partner' : 'faculty';
  ...
  private async autoCompleteFacultyStepForNewApplication(
    ...
    const decidedBy = primaryEmail
      ? await this.resolveFacultyUserIdByPrimaryEmail(primaryEmail)
      : null;
  ...
  private facultyEmailMatchesApplicationGate(
    ...
    const listingContact =
      sup && typeof sup.contact === 'string'
        ? this.normalizeEmail(sup.contact)
        : '';
    return listingContact !== '' && listingContact === email;
  }

  private async resolveFacultyUserIdByPrimaryEmail(
    email: string,
  ): Promise<string | null> {
  ...
  async facultyList(
    ...
    const email = this.normalizeEmail(facultyEmail);
    const assignment =
      await this.facultyUniversityScopeService.getAssignmentForFaculty(
        facultyUserId,
      );
    const orgNorm = assignment?.universityOrganization?.name
      ? this.facultyUniversityScopeService.normalizeOrgName(
          assignment.universityOrganization.name,
        )
      : null;
  ...
  async facultyApprove(
    ...
    if (app.internalStatus !== 'pending_faculty') {
      throw new BadRequestException(
        'Application is not awaiting faculty review',
      );
    }
    const opp = app.opportunity;
    if (!opp) {
  ...
  async partnerApprove(id: string, organizationId: string | null) {
    ...
    if (app.internalStatus !== 'pending_partner') {
      throw new BadRequestException(
        'Application is not awaiting partner review',
      );
    }
  ...
  async findPendingAdminApplicationsForQueue(): Promise<
    OpportunityApplication[]
  > {
    return this.appRepo.find({
      where: {
  ...
  async adminList(status?: string) {
    ...
    if (normalized !== 'pending' && normalized !== 'history') {
      throw new BadRequestException(
        'Only status=pending or status=history is supported for this listing',
      );
    }
    ...
    const rows =
      normalized === 'pending'
        ...
        : await this.appRepo.find({
            where: {
              internalStatus: In(ADMIN_HISTORY_STATUSES),
              withdrawnAt: IsNull(),
            },
            relations: [...relations],
            order: { createdAt: 'DESC' },
   ...
   */
  private async provisionApprovedApplicationSeats(
    app: OpportunityApplication,
  ): Promise<void> {
    const user =
      app.studentUser ||
      (await this.userRepo.findOne({ where: { id: app.studentUserId } }));
    if (!user) {
    ...
    const participationType =
      (payload['participation_type'] as string) || 'individual';
    const teamId = (payload['team_id'] as string) || undefined;
    const primaryFaculty =
      (payload['primary_faculty_email'] as string) || app.primaryFacultyEmail;
    const secondaryFaculty =
      (payload['secondary_faculty_email'] as string) ||
      app.secondaryFacultyEmail ||
      undefined;
    const normalizedPrimaryFaculty = primaryFaculty
      ? this.normalizeEmail(primaryFaculty)
      : undefined;
    const normalizedSecondaryFaculty = secondaryFaculty
      ? this.normalizeEmail(secondaryFaculty)
      : undefined;
    const contactPhone = (payload['contact_phone_e164'] as string) || undefined;
    const rawTeamMembers = Array.isArray(payload['team_members'])
    ...
    const leadAlreadyProvisioned =
      existingLead &&
      ['approved', 'verified', 'accepted', 'finalized'].includes(
        existingLead.status,
      );
  ...
  private async reconcileMissingTeamMemberSeatsForOpportunity(
    opportunityId: string,
  ): Promise<void> {
  ...
  async adminApprove(id: string, adminUserId: string) {
    ...
    if (
      app.internalStatus !== 'pending_admin' &&
      app.internalStatus !== 'pending_partner'
    ) {
   ...
   */
  async findApplyPayloadTeamMemberEmails(
    applicationId: string,
  ): Promise<string[]> {
    const app = await this.appRepo.findOne({
