...
describe('Section1 analytics visibility', () => {
    const sampleValues = {
        ...
        hours_status_in_header: { verified_hours: 12, required_hours: 16, percent: 75 },
        team_member_count: 4,
    ...
    it('returns all CIEL-visible fields for admin stakeholder', () => {
        const { fields } = filterSection1AnalyticsForStakeholder('ciel', sampleValues);
    ...
    it('hides restricted admin-only fields from partner stakeholder', () => {
        const sanitized = sanitizeRestrictedValuesForStakeholder('partner', sampleValues);
        const { fields } = filterSection1AnalyticsForStakeholder('partner', sanitized);
        expect(fields.project_title).toBe('Clean Campus Drive');
    ...
    it('hides personal identity fields from UN/Government stakeholder', () => {
        const sanitized = sanitizeRestrictedValuesForStakeholder('un_government', sampleValues);
        const { fields, meta } = filterSection1AnalyticsForStakeholder('un_government', sanitized);
    ...
    it('allows student-only operational button state for student stakeholder', () => {
        const { fields } = filterSection1AnalyticsForStakeholder('student', sampleValues);
        expect(fields.verify_identity_link_record_button_status).toEqual({ enabled: false });
    });
    ...
    it.each(stakeholders)('never leaks CNIC completion to %s unless allowed', (stakeholder) => {
        const sanitized = sanitizeRestrictedValuesForStakeholder(stakeholder, sampleValues);
        const { fields } = filterSection1AnalyticsForStakeholder(stakeholder, sanitized);
        if (stakeholder === 'ciel') {
            expect(fields.cnic_completion).toBeDefined();
        } else {
            expect(fields.cnic_completion).toBeUndefined();
