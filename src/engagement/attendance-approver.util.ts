...
export type AttendanceRoutingOverride = 'auto' | 'partner' | 'faculty';
...
export type OpportunityPartnerRoutingSource = {
  ...
  attendanceRoutingOverride?: AttendanceRoutingOverride | null;
...
export function extractPartnerContactEmailsForAttendance(
  ...
  const partnerOrg =
    opportunity.partner_organization &&
    typeof opportunity.partner_organization === 'object'
  ...
  const executingContext =
    opportunity.executing_context &&
    typeof opportunity.executing_context === 'object'
  ...
  return normalizeEmailList([
    typeof partnerOrg.official_email === 'string'
      ? partnerOrg.official_email
      : null,
    typeof partnerOrg.officialEmail === 'string'
      ? partnerOrg.officialEmail
      : null,
    typeof partnerOrg.email === 'string' ? partnerOrg.email : null,
    typeof partnerOrg.contact_email === 'string'
      ? partnerOrg.contact_email
      : null,
    typeof partnerOrg.contactEmail === 'string'
      ? partnerOrg.contactEmail
      : null,
    typeof supervision.partner_email === 'string'
      ? supervision.partner_email
      : null,
    typeof supervision.external_partner_email === 'string'
      ? supervision.external_partner_email
      : null,
    typeof externalCollab.official_email === 'string'
      ? externalCollab.official_email
      : null,
    typeof externalCollab.officialEmail === 'string'
      ? externalCollab.officialEmail
      : null,
    typeof contextPartner.official_email === 'string'
      ? contextPartner.official_email
      : null,
    typeof contextPartner.officialEmail === 'string'
      ? contextPartner.officialEmail
      : null,
  ]);
}
...
export function resolveEffectiveAttendanceApproverType(
  ...
  const overrideRaw = String(opportunity?.attendanceRoutingOverride ?? 'auto')
    .trim()
    .toLowerCase();
  if (overrideRaw === 'partner') {
    return 'partner';
  }
  if (overrideRaw === 'faculty') {
    return 'faculty';
  }
...
export function getParticipantFacultyEmails(participant: {
  ...
  return Array.from(
    new Set(raw.map((v) => (v || '').trim().toLowerCase()).filter(Boolean)),
  );
}
