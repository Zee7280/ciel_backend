...
describe('attendance partner routing', () => {
  ...
  it('honours admin attendance routing override before auto partner detection', () => {
    expect(
      resolveEffectiveAttendanceApproverType('faculty', {
    ...
    expect(
      resolveEffectiveAttendanceApproverType('partner', {
        attendanceRoutingOverride: 'faculty',
        partner_organization: { official_email: 'partner@ngo.org' },
      }),
    ).toBe('faculty');
  });

  it('uses auto routing when override is auto or unset', () => {
    expect(
      resolveEffectiveAttendanceApproverType('partner', {
        attendanceRoutingOverride: 'auto',
        isStudentCreated: true,
      }),
    ).toBe('faculty');
    expect(
