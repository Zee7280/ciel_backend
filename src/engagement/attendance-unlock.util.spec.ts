...
describe('attendance-unlock.util', () => {
  ...
  it('inherits team lead admin override for teammates on the same team', () => {
    ...
    const effective = resolveParticipationForAttendanceUnlock(member, [
      member,
      lead,
    ]);
  ...
  it('computes academic completion percent from participation fields', () => {
    expect(computeAcademicCompletionPercent(baseParticipation, baseUser)).toBe(
      100,
    );
    expect(
      computeAcademicCompletionPercent(
        { ...baseParticipation, academicProgram: null } as unknown as Participation,
        baseUser,
      ),
    ).toBe(80);
  });
  ...
  it('marks identity as verified only when all checks pass', () => {
    expect(resolveIdentityVerificationStatus(baseUser, baseParticipation)).toBe(
      'verified',
    );
    expect(
      resolveIdentityVerificationStatus(
