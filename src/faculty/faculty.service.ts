...
export class FacultyService {
  ...
  private approvalActionForRow(
    ...
    const namedSupervisor = this.opportunityMatchesNamedSupervisorPath(
      opp,
      facultyId,
      fe,
    );
  ...
  private studentLeadReportNeedsFacultyFirst(
  ...
  private reportImpactMetricsReleasedToFaculty(
   ...
   */
  private opportunityMatchesNamedSupervisorPath(
    opportunity: Opportunity,
    facultyId: string,
    fe: string,
  ): boolean {
    if (
      opportunity.facultyId != null &&
      String(opportunity.facultyId) === String(facultyId)
    ) {
      return true;
    }
    if (!fe) return false;
    ...
    if (sup && typeof sup === 'object') {
      const contact =
        typeof sup.contact === 'string'
          ? this.normalizeFacultyEmail(sup.contact)
          : '';
      const official =
        typeof sup.official_email === 'string'
          ? this.normalizeFacultyEmail(sup.official_email)
          : '';
    ...
    if (po && typeof po === 'object') {
      const pe =
        typeof po.official_email === 'string'
          ? this.normalizeFacultyEmail(po.official_email)
          : '';
  ...
  private approvalVisibilityForRow(
    ...
    const named = this.opportunityMatchesNamedSupervisorPath(
      opportunity,
      facultyId,
      fe,
    );
  ...
  private async resolveFacultyScopedContext(
    ...
    const personalIds = await this.resolveFacultyPersonalOpportunityIds(
      facultyId,
      facultyEmail,
    );
    const delegatedOppIds =
      await this.resolveDelegatedOpportunityIds(facultyId);
    ...
    const faculty_view_modes_available: FacultyDashboardViewMode[] =
      university_scope
   ...
   */
  private async resolveFacultyPersonalOpportunityIds(
    facultyId: string,
    facultyEmail: string,
  ): Promise<string[]> {
    const fe = this.normalizeFacultyEmail(facultyEmail);
    const oppQ = this.opportunitiesRepository
    ...
    if (fe) {
      ...
      const reportRows = await this.studentReportsRepository
        ...
        .where(
          `LOWER(TRIM(COALESCE(r.section1->>'faculty_supervisor_email', ''))) = :fe`,
          { fe },
        )
        .getRawMany();
      fromReports = reportRows
        .map(
          (r: { opportunityId?: string; projectId?: string }) =>
            r.opportunityId || r.projectId,
        )
  ...
  private async resolveFacultyScopedOpportunityIds(
    facultyId: string,
    facultyEmail: string,
  ): Promise<string[]> {
    const personal = await this.resolveFacultyPersonalOpportunityIds(
      facultyId,
      facultyEmail,
    );
  ...
  private async resolveDelegatedOpportunityIds(
    facultyId: string,
  ): Promise<string[]> {
    const orgId =
      await this.facultyUniversityScopeService.getDelegatedOrganizationId(
        facultyId,
      );
    if (!orgId) return [];
    return this.facultyUniversityScopeService.resolveOpportunityIdsForUniversityOrganization(
      orgId,
    );
  }
  ...
  private buildFacultyApprovalsQuery(
    ...
    const query = this.opportunitiesRepository
      ...
      .where(
        new Brackets((top) => {
          top.where(
            new Brackets((qb) => {
              qb.where('"opportunity"."facultyId"::text = :facultyId', {
                facultyId,
              });
              if (fe) {
                qb.orWhere(
    ...
    if (status === 'pending' || status === undefined || status === '') {
            ...
            .where(
              new Brackets((early) => {
                early
                  .where(
                    new Brackets((s) => {
                      s.where('opportunity.isStudentCreated = :isc', {
                        isc: true,
                      }).orWhere(
                  ...
                  .andWhere(
                    new Brackets((s) => {
                      s.where('opportunity.workflowStage = :facultyWorkflow', {
                        ...
                        .orWhere('opportunity.status = :pf', {
                          pf: 'pending_faculty',
                        })
                        .orWhere('opportunity.status = :pv', {
                          pv: 'pending_verification',
                        })
                        .orWhere(
                          'opportunity.facultyApprovalStatus = :facultyLinePending',
                          {
                            facultyLinePending: LINE_STATUS.PENDING,
                          },
                        )
                        .orWhere(
                          'opportunity.faculty_verification_status = :facultyStatusPending',
                          {
                            facultyStatusPending:
                              WORKFLOW_STAGE.PENDING_FACULTY,
                          },
                        );
                    }),
                  )
                  ...
                  .andWhere(
                    new Brackets((s) => {
                      s.where('opportunity.liaisonVerified = :lvp', {
                        lvp: false,
                      }).orWhere('opportunity.liaisonVerified IS NULL');
                    }),
                  );
              }),
            )
            .orWhere(
              new Brackets((partnerQ) => {
                ...
                partnerQ
                  ...
                  .andWhere('opportunity.requiresPartnerApproval = :pReq', {
                    pReq: true,
                  })
                  .andWhere('opportunity.partnerVerified = :pUnv', {
                    pUnv: false,
                  })
                  .andWhere(
                    new Brackets((facultyCleared) => {
                      facultyCleared
                        ...
                        .orWhere('opportunity.faculty_verified = :fvOk', {
                          fvOk: true,
                        });
                    }),
                  )
                  ...
                  .andWhere(
                    new Brackets((gate) => {
                      gate
                        .where('opportunity.workflowStage = :pGateWs', {
                        ...
                        .orWhere(
                          'opportunity.partnerApprovalStatus = :pGatePas',
                          {
      ...
      query
        ...
        .andWhere(
          new Brackets((qb) => {
            qb.where('opportunity.isStudentCreated = :isc', {
              isc: true,
            }).orWhere('opportunity.liaisonToken IS NOT NULL');
          }),
        )
        .andWhere(
          new Brackets((qb) => {
  ...
  private formatSdgLabel(opp: Opportunity): string {
    ...
    const num =
      typeof n === 'number' ? n : typeof n === 'string' ? parseInt(n, 10) : NaN;
  ...
  private pickCourseCode(opp: Opportunity): string | null {
    const al = opp.academic_linkage as
      | { course_code?: string; courseCode?: string }
      | undefined;
  ...
  private facultyAnalyticsParticipationDimensionFiltersActive(
  ...
  private facultyAnalyticsAnyFilterActive(
    query?: FacultyAnalyticsQueryDto,
  ): boolean {
  ...
  private compactFacultyAnalyticsFilterParams(
    query: FacultyAnalyticsQueryDto,
  ...
  private async narrowFacultyAnalyticsOpportunityIds(
    ...
    return opps
      .filter((o) => {
        ...
        return (
          code.includes(lower) || sem.includes(lower) || title.includes(lower)
        );
      })
      .map((o) => o.id);
  }
  ...
  private applyFacultyAnalyticsParticipationFilters(
    ...
    if (query.year_of_study?.trim()) {
      qb.andWhere('p.yearOfStudy = :fafYos', {
        fafYos: query.year_of_study.trim(),
      });
    }
  ...
  private appendFacultyTimesheetParticipationFilters(
    ...
    if (
      !query ||
      !this.facultyAnalyticsParticipationDimensionFiltersActive(query)
    )
      return;
  ...
  async getProjectDetail(
    ...
    const scopedIds = await this.resolveFacultyScopedOpportunityIds(
      facultyId,
      facultyEmail,
    );
  ...
  async getApprovals(facultyId: string, facultyEmail: string, status?: string) {
    ...
    const query = this.buildFacultyApprovalsQuery(
      facultyId,
      facultyEmail,
      status,
      delegatedOppIds,
    ...
    const formatted = await Promise.all(
      opportunities.map(async (opp) => {
        const approvalVisibility = this.approvalVisibilityForRow(
          opp,
          facultyId,
          facultyEmail,
          delegatedSet,
        ...
        const latestReport = opp.creatorId
          ...
          : null;
        const reportAwaitingFaculty = this.studentLeadReportNeedsFacultyFirst(
          latestReport,
          opp.creatorId,
        );
        ...
        const metricsReleased =
          this.reportImpactMetricsReleasedToFaculty(latestReport);
  ...
  async getDashboard(
    ...
    const delegatedForPendingQuery =
      effectiveView === 'personal' ? [] : delegatedOppIds;
            ...
            items: [
              {
                ...
                tone: 'warning',
                description:
                  'Student-created opportunities or reports waiting for faculty review.',
              },
            ],
          },
        },
    ...
    const reportSdgRows = await this.studentReportsRepository
      ...
      .addSelect(
        `COALESCE(r."opportunityId"::text, TRIM(r.project_id))`,
        'oppId',
      )
      .where("COALESCE(r.status, '') NOT IN (:...draftish)", {
        draftish: ['draft', 'rejected'],
      })
    ...
    const oppIdsWithReportSdg = new Set(
      reportSdgRows.map((r) => r.oppId).filter(Boolean),
    );
    ...
    const courses = await Promise.all(
      coursesBase.map(async (opp) => {
        ...
        const pending_grading = await this.studentReportsRepository
          ...
          .where(
            new Brackets((qb) => {
              qb.where('r."opportunityId"::text = :oid', {
                oid: opp.id,
              }).orWhere(
          ...
          .andWhere('(r.faculty_status IS NULL OR r.faculty_status = :fp)', {
            fp: 'pending',
          })
          .andWhere("COALESCE(r.status, '') NOT IN (:...ex)", {
            ex: ['draft', 'rejected'],
          })
          .getCount();
    ...
    const reportEvents = await this.studentReportsRepository
      ...
      .addSelect(
        'COALESCE(r.reportSubmittedAt, r.submission_date, r.updatedAt)',
        'report_recent_at',
      )
    ...
    for (const r of reportEvents) {
      ...
      const title =
        r.status === 'submitted' ||
        (r.reportSubmittedAt && r.status !== 'draft')
          ? 'Report submitted'
          : 'Report updated';
    ...
    for (const a of appEvents) {
      ...
      if (a.internalStatus === 'pending_faculty') {
        ...
        a.internalStatus === 'pending_partner' ||
        a.internalStatus === 'pending_admin'
      ) {
  ...
  async getImpactAnalytics(
    ...
    const filter_meta =
      query && this.facultyAnalyticsAnyFilterActive(query)
        ? {
            ...
            params: this.compactFacultyAnalyticsFilterParams(query),
          }
    ...
    const oppIds = await this.narrowFacultyAnalyticsOpportunityIds(
      scopedIds,
      query,
    );
    if (oppIds.length === 0) {
    ...
    const partDimActive = Boolean(
      query && this.facultyAnalyticsParticipationDimensionFiltersActive(query),
    );
    ...
    const totalStudents =
      Number((await totalStudentsQb.getRawOne<{ cnt: string }>())?.cnt ?? 0) ||
      0;
    ...
    const verificationRatePercent =
      totalStudents === 0
        ? 0
        : Math.round((100 * verifiedStudents) / totalStudents);
    ...
    const participationsQb = this.participationRepository
      ...
      .andWhere('p.status IN (:...st)', { st })
      .andWhere('p.student_id IS NOT NULL');
    this.applyFacultyAnalyticsParticipationFilters(
      participationsQb,
      query,
      'u',
    );
    ...
    const averageTeamSize =
      totalTeams > 0
        ? Math.round((teamParticipants / totalTeams) * 10) / 10
        : 0;
    ...
    const ceStudentsRowQb = this.participationRepository
      ...
      .andWhere('p.status IN (:...st)', { st })
      .andWhere('p.student_id IS NOT NULL')
      .andWhere('p.academicIntegrationType IN (:...ce)', {
        ce: [...COURSE_LINKED_ACADEMIC_TYPES],
      });
    this.applyFacultyAnalyticsParticipationFilters(ceStudentsRowQb, query, 'u');
    const ceStudentCount =
      Number((await ceStudentsRowQb.getRawOne<{ cnt: string }>())?.cnt ?? 0) ||
      0;
    const courseLinkedCeRatioPercent =
      totalStudents === 0
        ? 0
        : Math.round((100 * ceStudentCount) / totalStudents);
    ...
    const hoursRowQb = this.timesheetsRepository
      ...
      .where('t.status = :vs', { vs: 'verified' })
      .andWhere('t.opportunityId IN (:...ids)', idParams);
    this.appendFacultyTimesheetParticipationFilters(
      hoursRowQb,
      't',
      oppIds,
      st,
    ...
    const hoursVerified =
      Number((await hoursRowQb.getRawOne<{ sum: string }>())?.sum ?? 0) || 0;
    ...
    const projectsCompleted =
      Number(
        (await projectsCompletedRowQb.getRawOne<{ cnt: string }>())?.cnt ?? 0,
      ) || 0;
    ...
    const trendRows = await trendRowsQb
      .groupBy('bucket')
      .orderBy('bucket', 'ASC')
      .getRawMany<{
        bucket: Date;
    ...
    this.appendFacultyReportParticipationFilters(
      reportSdgQb,
      'r',
      oppIds,
      st,
    ...
    const reportSdgRows = await reportSdgQb.getRawMany<{
      g: number;
      oppId: string;
    }>();
    ...
    const avgImpactScore =
      Math.round((Number(avgRow?.avg ?? 0) || 0) * 10) / 10;
